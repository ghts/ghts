...
func go_TR호출_도우미(ch초기화, ch종료 chan lib.T신호) (에러 error) {
	defer lib.S예외처리{M에러: &에러, M함수: func() { ch종료 <- lib.P신호_에러 }}.S실행()
...
func go_TR콜백_처리(ch초기화, ch종료 chan lib.T신호) (에러 error) {
	...
	for {
		...
		default:
			...
			i값 := 수신값.S해석기(st.F바이트_변환값_해석).G해석값_단순형(0)
...
func go실시간_데이터_처리(ch초기화, ch종료 chan lib.T신호) (에러 error) {
	defer lib.S예외처리{M에러: &에러, M함수: func() { ch종료 <- lib.P신호_에러 }}.S실행()
	...
	for {
		...
		default:
			...
			실시간_데이터 := 수신값.S해석기(st.F바이트_변환값_해석).G해석값_단순형(0).(lib.I_TR코드)
