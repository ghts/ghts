...
func F테스트_같음(테스트 testing.TB, 값 interface{}, 비교값_모음 ...interface{}) {
	// 종료할 때 문자열 출력 일시정지 상태를 원상 회복
	문자열_출력_일시정지_중_원본 := F문자열_출력_일시정지_중()
	...
	for _, 비교값 := range 비교값_모음 {
		...
		if (문자열 == "<nil>" && 비교값_문자열 == "<nil>") ||
			reflect.DeepEqual(값, 비교값) {
			// 같은 값 발견
			return
		}
	}
	...
	변수값_모음 := []interface{}{값}
	변수값_모음 = append(변수값_모음, 비교값_모음...)

	F호출경로_건너뛴_문자열_출력(1, "같은 값을 발견하지 못함.\n%v",
		F변수_내역_문자열(변수값_모음...))
	테스트.FailNow()
}

func F테스트_다름(테스트 testing.TB, 값 interface{}, 비교값_모음 ...interface{}) {
	// 종료할 때 문자열 출력 일시정지 상태를 원상 회복
	문자열_출력_일시정지_중_원본 := F문자열_출력_일시정지_중()
	...
	for _, 비교값 := range 비교값_모음 {
		...
		if (문자열 == "<nil>" && 비교값_문자열 == "<nil>") ||
			...
			F호출경로_건너뛴_문자열_출력(1, "서로 같은 값 존재함.\n%v",
				F변수_내역_문자열(변수값_모음...))
			테스트.FailNow()
		}
	}
...
func F변수값_확인(값_모음 ...interface{}) {
	fmt.Println(F소스코드_위치(1), "변수값 확인")
	fmt.Println(F변수_내역_문자열(값_모음...))
}
				...
				if 필드.Kind() == reflect.Struct {
					버퍼.WriteString(
						F포맷된_문자열("%v 필드명 : '%v', 형식 : %v,  값 : %v\n",
							i, 필드_자료형.Name, 필드.Type(), 필드.Interface()))
				} else {
					버퍼.WriteString(
						F포맷된_문자열("%v 필드명 : '%v', 형식 : %v,  값 : %v\n",
							i, 필드_자료형.Name, 필드.Type(), 필드))
				}
			}
