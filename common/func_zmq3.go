...
func F메시지_송신(소켓 *zmq3.Socket, 내용 ...interface{}) error {
...
func F에러_메시지_송신(소켓 *zmq3.Socket, 에러 error) error {
	return F메시지_송신(소켓, P메시지_에러, 에러.Error())
}

// zmq소켓에서 온 질의 메시지를 Go채널로 중계해 주고,
// 그 회신을 다시 zmq소켓으로 전달해 주는 함수.
func F_zmq소켓_Go채널_중계(zmq소켓 *zmq3.Socket, Go채널 chan I질의) (에러 error) {
	defer func() {
		r := recover()
