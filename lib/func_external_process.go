...
func F파이썬_스크립트_실행(스크립트_경로 string, 실행옵션 ...interface{}) (프로세스ID int, 에러 error) {
	defer S예외처리{M함수: func() { 프로세스ID = -1 }}.S실행()

	if 파이썬_경로.G값() == "" {
		파일경로 := F확인(F파일_검색(F홈_디렉토리(), "python.exe")).(string)
		파이썬_경로.S값(파일경로)
	}
...
func F외부_프로세스_실행(실행화일_경로 string, 실행옵션_모음 ...interface{}) (프로세스ID int, 에러 error) {
	defer S예외처리{M에러: &에러, M함수: func() { 프로세스ID = -1 }}.S실행()

	if !F파일_존재함(실행화일_경로) {
		panic(New에러("실행화일이 존재하지 않습니다.\n%s", 실행화일_경로))
	}

	if !프로세스ID_목록_파일_초기화_완료.G값() {
		if 에러 := 프로세스ID_목록_파일_초기화_완료.S값(true); 에러 == nil {
			if 수량 := F확인(f잔류_프로세스_정리_및_초기화()).(int); 수량 > 0 {
				F문자열_출력("%v개의 잔류 프로세스를 정리했습니다.", 수량)
			}
		}
	}
	...
	case 에러 = <-ch에러:
		return -1, 에러
	case <-time.After(P10초):
		return -1, New에러("외부프로세스 실행 타임아웃. %v", 실행화일_경로)
	}
}

func f외부_프로세스_생성(ch프로세스ID chan int, ch에러 chan error, 실행화일_경로 string, 실행옵션_모음 ...interface{}) (에러 error) {
	defer S예외처리{M에러: &에러, M함수: func() { ch에러 <- 에러 }}.S실행()

	외부_명령어 := exec.Command(실행화일_경로, F2문자열_모음(실행옵션_모음)...)
...
func f프로세스ID_파일_읽기() (맵 map[int]S비어있음, 에러 error) {
	defer S예외처리{M에러: &에러, M함수: func() { 맵 = nil }}.S실행()

	if 존재함 := F확인(f프로세스ID_파일_존재함()).(bool); !존재함 {
		F확인(f프로세스ID_파일_초기화())
	}

	프로세스ID_저장소 := make(map[int]S비어있음)
	F확인(F파일에서_값_읽기(&프로세스ID_저장소, p프로세스ID_목록_파일명, nil))
...
func f프로세스ID_파일_초기화() error {
	비어있는_맵 := make(map[int]S비어있음)

	return F파일에_값_저장(비어있는_맵, p프로세스ID_목록_파일명, nil)
}

func f잔류_프로세스_정리_및_초기화() (수량 int, 에러 error) {
	defer S예외처리{M에러: &에러}.S실행()

	프로세스ID_저장소 := F확인(f프로세스ID_파일_읽기()).(map[int]S비어있음)
