...
func TestF2주소(t *testing.T) {
	t.Parallel()

	주소값_모음 := []T주소{P주소_주소정보, P주소_종목정보,
		P주소_가격정보, P주소_가격정보_입수, P주소_가격정보_배포}

	for _, 주소_원본 := range 주소값_모음 {
		주소, 에러 := F2주소(주소_원본.G값())
		F테스트_에러없음(t, 에러)
		F테스트_같음(t, 주소, 주소_원본)
	}

	주소, 에러 := F2주소("변환 불가능한 M값")
	F테스트_에러발생(t, 에러)
	F테스트_같음(t, 주소, T주소(-1))
}
...
func TestF바이트_변환값_해석(t *testing.T) {
	...
	F테스트_같음(t, pInt, F자료형_문자열_단순형(0))
	F테스트_같음(t, pInt64, F자료형_문자열_단순형(int64(0)))
	F테스트_같음(t, pFloat64, F자료형_문자열_단순형(float64(0.0)))
	F테스트_같음(t, pBool, F자료형_문자열_단순형(true))
	F테스트_같음(t, pString, F자료형_문자열_단순형(""))
	F테스트_같음(t, pStringArray, F자료형_문자열_단순형([]string{}))
	F테스트_같음(t, pTime, F자료형_문자열_단순형(time.Time{}))
	F테스트_같음(t, pT신호, F자료형_문자열_단순형(T신호(0)))
	F테스트_같음(t, pS질의값_기본형, F자료형_문자열_단순형(S질의값_기본형{}))
	F테스트_같음(t, pS질의값_문자열, F자료형_문자열_단순형(S질의값_문자열{}))
	F테스트_같음(t, pS질의값_문자열_모음, F자료형_문자열_단순형(S질의값_문자열_모음{}))
	F테스트_같음(t, pS질의값_바이트_변환, F자료형_문자열_단순형(S질의값_바이트_변환{}))
	F테스트_같음(t, pS질의값_바이트_변환_모음, F자료형_문자열_단순형(S질의값_바이트_변환_모음{}))
	F테스트_같음(t, pS질의값_단일_종목, F자료형_문자열_단순형(S질의값_단일_종목{}))
	F테스트_같음(t, pS질의값_복수_종목, F자료형_문자열_단순형(S질의값_복수_종목{}))
	F테스트_같음(t, pS질의값_정상_주문, F자료형_문자열_단순형(S질의값_정상_주문{}))
	F테스트_같음(t, pS질의값_정정_주문, F자료형_문자열_단순형(S질의값_정정_주문{}))
	F테스트_같음(t, pS질의값_취소_주문, F자료형_문자열_단순형(S질의값_취소_주문{}))
}
