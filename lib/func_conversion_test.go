...
func TestF포맷_시간대_포함(t *testing.T) {
	t.Parallel()

	// Go 시간대 토큰 8종(MST, Z, Z0700, Z07:00, Z07, -0700, -07:00, -07)은 모두 탐지해야 한다.
	시간대_포함_포맷_모음 := []string{
	...
	for _, 포맷 := range 시간대_포함_포맷_모음 {
		F테스트_참임(t, f포맷_시간대_포함(포맷), 포맷)
	}

	// 시간대 토큰이 없는 포맷(시간대 없음, 소문자 리터럴, "+0700" 리터럴)은 탐지되면 안 된다.
	시간대_없는_포맷_모음 := []string{
		"2006-01-02 15:04:05",
		"20060102",
		"2006/01/02",
		"20060102 150405",
		"20060102150405",
		"20060102150405.99999999",
		"15:04:05",
		"15:04:05 z",
		"15:04:05 mst",
		"15:04:05+0700",
	}
	for _, 포맷 := range 시간대_없는_포맷_모음 {
		F테스트_거짓임(t, f포맷_시간대_포함(포맷), 포맷)
	}
}
