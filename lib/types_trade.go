...
func (s S종목별_일일_가격정보_모음) MAX(윈도우_크기 int) []float64 {
	...
	return F고가(전일_고가_모음, 윈도우_크기)
}

func (s S종목별_일일_가격정보_모음) MIN(윈도우_크기 int) []float64 {
	전일_저가_모음 := s.G전일_저가()

	return F저가(전일_저가_모음, 윈도우_크기)
}
...
func (s S종목별_일일_가격정보_모음) G볼린저_밴드_폭_Z점수(윈도우_크기 int, 표준편차_배율 float64) []float64 {
	// Look Ahead Bias를 방지하기 위해서 전일 종가 기준으로 함.
	볼린저_밴드_폭 := F볼린저_밴드_폭(s.G전일_종가(), 윈도우_크기, 표준편차_배율)

	return Z점수(볼린저_밴드_폭, 윈도우_크기)
}

// systrader79가 언급한 볼린저 밴드 폭 Z점수 보완 지수
func (s S종목별_일일_가격정보_모음) G볼린저_밴드_폭_고점_대비_비율(윈도우_크기 int, 표준편차_배율 float64) []float64 {
	// Look Ahead Bias를 방지하기 위해서 전일 종가 기준으로 함.
	볼린저_밴드_폭 := F볼린저_밴드_폭(s.G전일_종가(), 윈도우_크기, 표준편차_배율)
