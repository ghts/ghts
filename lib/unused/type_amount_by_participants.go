...
func F종목별_매매주체_동향_DB읽기(db *sql.DB, 종목코드 string, 시작일 time.Time) (값_모음 []*S종목별_매매주체_동향, 에러 error) {
	...
	주말 := lb.F금일().Weekday() == time.Saturday || lb.F금일().Weekday() == time.Sunday
	금일 := lb.F일자2정수(lb.F금일())
...
func F종목별_매매주체_동향_모음_DB저장(db *sql.DB, 값_모음 []*S종목별_매매주체_동향) (에러 error) {
	...
	for _, 값 := range 값_모음 {
		...
		lb.F확인2(stmt생성.Exec(값.M종목코드, 값.M일자))
		lb.F확인2(stmt수정.Exec(값.M기관_순매수액, 값.M외국인_순매수액, 값.M개인_순매수액, 값.M종목코드, 값.M일자))
	}
