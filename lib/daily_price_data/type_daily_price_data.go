...
func (s S종목별_일일_가격정보_모음) G이전_종가(이전_일수 int) float64 {
	종가_모음 := s.G종가_모음()
	인덱스 := lib.F최대값_정수(0, len(종가_모음)-이전_일수-1)
...
func (s S종목별_일일_가격정보_모음) G기간_고점_종가(시작_룩백_기간, 종료_룩백_기간 int) float64 {
	종가_모음 := s.G종가_모음()
	시작_인덱스 := len(종가_모음) - lib.F중간값_정수(시작_룩백_기간, 종료_룩백_기간, len(종가_모음)-1) - 1
	종료_인덱스 := len(종가_모음) - lib.F중간값_정수(0, 시작_룩백_기간, 종료_룩백_기간) - 1
...
func (s S종목별_일일_가격정보_모음) G평균_거래량(윈도우_크기 int) float64 {
	거래량_모음_전체 := s.g거래량_모음()
	시작_인덱스 := lib.F조건부_정수(len(거래량_모음_전체) < 윈도우_크기, 0, len(거래량_모음_전체)-윈도우_크기)
...
func (s S종목별_일일_가격정보_모음) G기간_상승일_최대_거래량(시작, 기간 int) float64 {
	최대_거래량 := 0.0
	시작_인덱스 := lib.F조건부_정수(len(s.M저장소) < 시작+기간, 0, len(s.M저장소)-시작-기간)
	종료_인덱스 := lib.F조건부_정수(len(s.M저장소) < 시작, 0, len(s.M저장소)-시작)
...
func (s S종목별_일일_가격정보_모음) G기간_순_거래량(윈도우_크기 int) float64 {
	순_거래량 := 0.0
	시작_인덱스 := lib.F조건부_정수(len(s.M저장소) < 윈도우_크기, 0, len(s.M저장소)-윈도우_크기)
...
func (s S종목별_일일_가격정보_모음) G평균_거래_금액(윈도우_크기 int) float64 {
	...
	return lib.F평균(거래_금액_모음) / 100_000_000 // 억 단위.
}
...
func (s S종목별_일일_가격정보_모음) ATR(윈도우_크기 int) float64 {
	...
	for i := 2; i < len(s.M저장소); i++ {
		고가 := lib.F최대값([]float64{s.M저장소[i].M고가, s.M저장소[i-1].M종가})
		저가 := lib.F최소값([]float64{s.M저장소[i].M저가, s.M저장소[i-1].M종가})
