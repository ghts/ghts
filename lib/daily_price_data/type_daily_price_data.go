...
func (s S종목별_일일_가격정보_모음) G일일_추세_점수() float64 {
	return s.g일일_추세_점수_도우미(len(s.M저장소) - 1)
}

func (s S종목별_일일_가격정보_모음) g일일_추세_점수_도우미(기준_인덱스 int) float64 {
	기준가 := s.M저장소[기준_인덱스].M종가
	반복_횟수 := lib.F최소값_정수(252, len(s.M저장소))
	카운터 := 0
