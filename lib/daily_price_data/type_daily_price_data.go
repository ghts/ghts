...
func (s S종목별_일일_가격정보_모음) G거래_금액_민감도(윈도우_크기 int) float64 {
	...
	for i := 0; i < 윈도우_크기; i++ {
		일_수익율_절대값 := math.Abs(종가_모음[i] - 전일_종가_모음[i]) / 전일_종가_모음[i]
...
func (s S종목별_일일_가격정보_모음) G일_최대_수익율(윈도우_크기, 샘플_수량 int) float64 {
	...
	for i := 0; i < 윈도우_크기; i++ {
		일_수익율_모음[i] = (종가_모음[i] - 전일_종가_모음[i]) / 전일_종가_모음[i]
	}
...
func (s S종목별_일일_가격정보_모음) G일일_추세_점수(윈도우_크기 int) float64 {
	종가_모음 := s.G종가_모음()
	return s.g일일_추세_점수_도우미(종가_모음, 윈도우_크기, len(종가_모음)-1)
}

func (s S종목별_일일_가격정보_모음) G평활화_일일_추세_점수(추세_점수_윈도우_크기, 평활화_윈도우_크기 int) float64 {
	종가_모음 := s.G종가_모음()
	일일_추세_점수_모음 := make([]float64, 평활화_윈도우_크기)

	for i := 0; i < 평활화_윈도우_크기; i++ {
		일일_추세_점수_모음[i] = s.g일일_추세_점수_도우미(종가_모음, 추세_점수_윈도우_크기, len(종가_모음)-평활화_윈도우_크기+i)
	}

	합계 := lib.F합계(일일_추세_점수_모음)
	최대값 := lib.F최대값_실수(일일_추세_점수_모음)
	최소값 := lib.F최소값_실수(일일_추세_점수_모음)
	필터된_평균 := (합계 - 최대값 - 최소값) / float64(평활화_윈도우_크기-2)

	return 필터된_평균
}

func (s S종목별_일일_가격정보_모음) G일일_추세_점수_SMA(추세_점수_윈도우_크기, 평활화_윈도우_크기 int) float64 {
	종가_모음 := s.G종가_모음()
	일일_추세_점수_모음 := make([]float64, 평활화_윈도우_크기)
	...
	return lib.F평균(일일_추세_점수_모음)
}

func (s S종목별_일일_가격정보_모음) G일일_추세_점수_EMA(추세_점수_윈도우_크기, 평활화_윈도우_크기 int) float64 {
	종가_모음 := s.G종가_모음()
	일일_추세_점수_모음 := make([]float64, 3*평활화_윈도우_크기)

	for i := 0; i < 3*평활화_윈도우_크기; i++ {
		일일_추세_점수_모음[i] = s.g일일_추세_점수_도우미(종가_모음, 추세_점수_윈도우_크기, len(종가_모음)-3*평활화_윈도우_크기+i)
	}

	평활화_일일_추세_점수_모음 := trade.F지수_이동_평균(일일_추세_점수_모음, 평활화_윈도우_크기)

	return 평활화_일일_추세_점수_모음[len(평활화_일일_추세_점수_모음)-1]
}
