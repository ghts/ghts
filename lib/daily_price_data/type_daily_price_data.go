...
func (s S종목별_일일_가격정보_모음) ATR_해외_원자재_ETF_ETN(윈도우_크기 int) float64 {
	...
	for i := 3; i < len(s.M저장소); i++ {
		고가_후보_1 := s.M저장소[i].M고가
		고가_후보_2 := s.M저장소[i-1].M고가
		고가_후보_3 := s.M저장소[i-2].M고가
		고가 := lib.F최대값_실수([]float64{고가_후보_1, 고가_후보_2, 고가_후보_3})

		저가_후보_1 := s.M저장소[i].M저가
		저가_후보_2 := s.M저장소[i-1].M저가
		저가_후보_3 := s.M저장소[i-2].M저가
		저가 := lib.F최소값_실수([]float64{저가_후보_1, 저가_후보_2, 저가_후보_3})
...
func (s S종목별_일일_가격정보_모음) G평활화_일일_추세_점수(추세_점수_윈도우_크기, 평활화_윈도우_크기 int) float64 {
	...
	lib.F체크포인트(s.G종목코드(), s.G일일_추세_점수(추세_점수_윈도우_크기), 평활화_일일_추세_점수_모음)
...
func (s *S일일_가격_백테스트_도우미) TrueRange_모음() []float64 {
	...
	for i := 2; i < s.d.Len(); i++ {
		...
		값2 := math.Abs(s.d.M저장소[i-1].M고가 - s.d.M저장소[i-2].M저가)
		값3 := math.Abs(s.d.M저장소[i-2].M고가 - s.d.M저장소[i-1].M저가)
...
func (s *S일일_가격_백테스트_도우미) TrueRange_원자재_해외_ETF_ETN_모음() []float64 {
	...
	for i := 3; i < s.d.Len(); i++ {
		...
		고가_후보_1 := s.d.M저장소[i-1].M고가
		고가_후보_2 := s.d.M저장소[i-2].M고가
		고가_후보_3 := s.d.M저장소[i-3].M고가
		고가 := lib.F최대값_실수([]float64{고가_후보_1, 고가_후보_2, 고가_후보_3})

		저가_후보_1 := s.d.M저장소[i-1].M저가
		저가_후보_2 := s.d.M저장소[i-2].M저가
		저가_후보_3 := s.d.M저장소[i-3].M저가
		저가 := lib.F최소값_실수([]float64{저가_후보_1, 저가_후보_2, 저가_후보_3})
