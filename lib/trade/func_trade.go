...
func F이동_범위_최대값(값_모음 []float64, 윈도우_크기 int) []float64 {
	if len(값_모음) == 0 {
		panic(lb.New에러("F이동_범위_최대값 : 비어있는 값_모음."))
	} else if 윈도우_크기 <= 0 {
		panic(lb.New에러("F이동_범위_최대값 : 잘못된 윈도우_크기 '%d'", 윈도우_크기))
	}

	윈도우_크기 = lb.F최소값(len(값_모음), 윈도우_크기)
	최대값_모음 := make([]float64, len(값_모음))
	최대값_모음[0] = 값_모음[0]

	for i := 1; i < 윈도우_크기; i++ {
		최대값_모음[i] = lb.F최대값(값_모음[:i+1]...)
	}

	for i := 윈도우_크기; i < len(값_모음); i++ {
		최대값_모음[i] = lb.F최대값(값_모음[i-윈도우_크기+1 : i+1]...)
	}
...
func F이동_범위_최소값(값_모음 []float64, 윈도우_크기 int) []float64 {
	if len(값_모음) == 0 {
		panic(lb.New에러("F이동_범위_최소값 : 비어있는 값_모음."))
	} else if 윈도우_크기 <= 0 {
		panic(lb.New에러("F이동_범위_최소값 : 잘못된 윈도우_크기 '%d'", 윈도우_크기))
	}

	윈도우_크기 = lb.F최소값(len(값_모음), 윈도우_크기)
	최소값_모음 := make([]float64, len(값_모음))
	최소값_모음[0] = 값_모음[0]

	for i := 1; i < 윈도우_크기; i++ {
		최소값_모음[i] = lb.F최소값(값_모음[:i+1]...)
	}

	for i := 윈도우_크기; i < len(값_모음); i++ {
		최소값_모음[i] = lb.F최소값(값_모음[i-윈도우_크기+1 : i+1]...)
	}
...
func F지수_이동_평균(값_모음 []float64, 윈도우_크기 int) []float64 {
	if 윈도우_크기 <= 0 {
		panic(lb.New에러("F지수_이동_평균 : 윈도우 크기 0"))
	} else if len(값_모음) == 0 {
		panic(lb.New에러("F지수_이동_평균 : len(값_모음) 0"))
	}
...
func F가중_이동_평균(값_모음, 가중치 []float64, 윈도우_크기 int) []float64 {
	if len(값_모음) != len(가중치) {
		panic(lb.New에러("서로 다른 길이 : %v %v", len(값_모음), len(가중치)))
	}

	길이 := len(가중치)
	윈도우_크기 = lb.F최소값(윈도우_크기, 길이)
	가중_이동_평균 := make([]float64, 길이)
	...
	for i := 0; i < 윈도우_크기; i++ {
		가중치_합계 += 가중치[i]
		가중_값_합계 += 값_모음[i] * 가중치[i]

		if 가중치_합계 == 0 {
			가중_이동_평균[i] = 0
		} else {
			가중_이동_평균[i] = 가중_값_합계 / 가중치_합계
		}
	}

	// 정상 구간
	가중치_합계 = lb.F합계(가중치[:윈도우_크기]...)
...
// F와일더_이동_평균 : Wilder 평활화(RMA).
// 인덱스 윈도우_크기-1까지 : 확장 윈도우 평균(워밍업),
// 이후 : 평활화[i] = (평활화[i-1]*(w-1) + 값[i]) / w  (승수 1/w).
func F와일더_이동_평균(값_모음 []float64, 윈도우_크기 int) []float64 {
	윈도우_크기 = lb.F최소값(윈도우_크기, len(값_모음))

	평활화_모음 := make([]float64, len(값_모음))
	...
	for i := 0; i < 윈도우_크기; i++ {
		합계 += 값_모음[i]
		평활화_모음[i] = 합계 / float64(i+1)
	}

	분모 := float64(윈도우_크기)
	나머지 := float64(윈도우_크기 - 1)

	for i := 윈도우_크기; i < len(값_모음); i++ {
		평활화_모음[i] = (평활화_모음[i-1]*나머지 + 값_모음[i]) / 분모
	}
