...
func TestS일일_가격정보_모음(t *testing.T) {
	일일_가격정보_1 := New일일_가격정보(
		"000000",
		F2포맷된_일자_단순형("2006-01-02", "2000-01-01"),
		1, 2, 3, 4, 5)

	일일_가격정보_2 := New일일_가격정보(
		"000000",
		F2포맷된_일자_단순형("2006-01-02", "2000-01-02"),
		2, 3, 4, 5, 6)

	일일_가격정보_3 := New일일_가격정보(
		"111111",
		F2포맷된_일자_단순형("2006-01-02", "2000-01-03"),
		3, 4, 5, 6, 7)
	...
	F테스트_에러없음(t, 에러)
	F테스트_같음(t, len(종목별_일일_가격정보_모음.M저장소), 2)
	F테스트_같음(t, len(종목별_일일_가격정보_모음.인덱스), 2)
	F테스트_참임(t, 종목별_일일_가격정보_모음.M저장소[0].M일자.Before(종목별_일일_가격정보_모음.M저장소[1].M일자))
	F테스트_같음(t, 종목별_일일_가격정보_모음.G종목코드(), "000000")

	값, 에러 := 종목별_일일_가격정보_모음.G값(일일_가격정보_1.M일자)
	F테스트_에러없음(t, 에러)
	F테스트_같음(t, 값.M일자, 일일_가격정보_1.M일자)

	값, 에러 = 종목별_일일_가격정보_모음.G값(일일_가격정보_3.M일자)
	F테스트_에러발생(t, 에러)
	F테스트_같음(t, 값, nil)

	새_값, 에러 := 종목별_일일_가격정보_모음.S추가(일일_가격정보_3)
	F테스트_에러발생(t, 에러)
	F테스트_같음(t, 새_값, nil)
