...
func f볼린저_밴드_도우미(값_모음, 이동_평균_모음 []float64, 윈도우_크기 int, 표준편차_배율 float64) []float64 {
	편차_제곱_모음 := make([]float64, len(값_모음))
	표준_편차_모음 := make([]float64, len(값_모음))
	볼린저_밴드_모음 := make([]float64, len(값_모음))
	...
	for i := 0; i < len(값_모음); i++ {
		볼린저_밴드_모음[i] = 이동_평균_모음[i] + 표준편차_배율*표준_편차_모음[i]
	}
