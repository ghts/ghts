...
func F임의_참거짓() bool {
	return F임의_범위_이내_정수값(0, 1) == 0
}

func F임의_문자열(최소_길이, 최대_길이 int) string {
	최소_길이, 최대_길이 = F최대N최소(최소_길이, 최대_길이)
	길이 := F임의_범위_이내_정수값(최소_길이, 최대_길이)
	버퍼 := new(bytes.Buffer)
	...
	for i := 0; i < 길이; i++ {
		버퍼.WriteString(문자열_모음[F임의_범위_이내_정수값(0, len(문자열_모음)-1)])
	}
...
func F임의_범위_이내_정수값(값1, 값2 int) int {
	최대값, 최소값 := F최대N최소(값1, 값2) // 인수 순서 무관

	폭 := 최대값 - 최소값
	F조건부_패닉(폭 < 0 || 폭 == math.MaxInt, "범위 폭 오버플로. 최소값 : %v, 최대값 : %v", 최소값, 최대값)

	// Intn(폭+1)은 [0, 폭]을 균등하게 반환하므로 두 끝값 모두 포함됨.
	return 최소값 + F임의값_생성기().Intn(폭+1)
}
...
func F임의_변환_형식() T변환 {
	...
	return 변환형식_모음[F임의값_생성기().Intn(len(변환형식_모음))]
}
...
func F임의_샘플_종목() *S종목 {
	...
	return 종목_모음[F임의_범위_이내_정수값(0, len(종목_모음)-1)].G복제본()
}
...
func F종목_추출(종목_모음 []*S종목, 수량 int) (추출_종목_모음 []*S종목) {
	defer S예외처리{M에러_실행: func() { 추출_종목_모음 = nil }}.S실행()
...
func F종목코드_추출(종목_모음 []*S종목, 수량 int) (종목코드_모음 []string) {
	defer S예외처리{M에러_실행: func() { 종목코드_모음 = nil }}.S실행()
