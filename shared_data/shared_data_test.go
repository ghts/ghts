...
func TestF공용_데이터_Go루틴_주소정보(테스트 *testing.T) {
	...
	회신 := 공용.New질의(공용.P메시지_초기화).G회신(ch제어_공용_데이터_Go루틴)
...
func TestF공용_데이터_Go루틴_종목별_보유량(테스트 *testing.T) {
	...
	for _, 종목 := range 종목_모음 {
		...
		회신 = 공용.New질의(공용.P메시지_GET, 공용.P포지션_롱).G회신(Ch종목별_보유량)
		공용.F테스트_에러없음(테스트, 회신.G에러())
		공용.F테스트_같음(테스트, 회신.G길이(), 1)
		공용.F테스트_같음(테스트, 회신.G내용(0), 공용.F2문자열(종목별_보유량.G롱포지션()))
		
		회신 = 공용.New질의(공용.P메시지_GET, 공용.P포지션_숏).G회신(Ch종목별_보유량)
		공용.F테스트_에러없음(테스트, 회신.G에러())
		공용.F테스트_같음(테스트, 회신.G길이(), 1)
		공용.F테스트_같음(테스트, 회신.G내용(0), 공용.F2문자열(종목별_보유량.G숏포지션()))
	}
	
	회신 = 공용.New질의(공용.P메시지_GET, "존재하지 않는 종목코드", 공용.P포지션_롱).G회신(Ch종목)
	공용.F테스트_에러발생(테스트, 회신.G에러())
	공용.F테스트_같음(테스트, 회신.G구분(), 공용.P메시지_에러)
	공용.F테스트_같음(테스트, 회신.G길이(), 0)
	
	회신 = 공용.New질의(공용.P메시지_GET, "존재하지 않는 종목코드", 공용.P포지션_숏).G회신(Ch종목)
	공용.F테스트_에러발생(테스트, 회신.G에러())
	공용.F테스트_같음(테스트, 회신.G구분(), 공용.P메시지_에러)
	공용.F테스트_같음(테스트, 회신.G길이(), 0)
	
	회신 = 공용.New질의(공용.P메시지_GET, 종목_모음[0], "잘못된 포지션 종류").G회신(Ch종목)
	공용.F테스트_에러발생(테스트, 회신.G에러())
	공용.F테스트_같음(테스트, 회신.G구분(), 공용.P메시지_에러)
	공용.F테스트_같음(테스트, 회신.G길이(), 0)
	
	회신 = 공용.New질의(공용.P메시지_SET, "존재하지 않는 종목코드", 공용.P포지션_롱, 1).G회신(Ch종목)
	공용.F테스트_에러발생(테스트, 회신.G에러())
	공용.F테스트_같음(테스트, 회신.G구분(), 공용.P메시지_에러)
	공용.F테스트_같음(테스트, 회신.G길이(), 0)
	
	회신 = 공용.New질의(공용.P메시지_SET, "존재하지 않는 종목코드", 공용.P포지션_숏, 1).G회신(Ch종목)
	공용.F테스트_에러발생(테스트, 회신.G에러())
	공용.F테스트_같음(테스트, 회신.G구분(), 공용.P메시지_에러)
	공용.F테스트_같음(테스트, 회신.G길이(), 0)
	
	회신 = 공용.New질의(공용.P메시지_GET, 종목_모음[0], "잘못된 포지션 종류", 1).G회신(Ch종목)
	공용.F테스트_에러발생(테스트, 회신.G에러())
	공용.F테스트_같음(테스트, 회신.G구분(), 공용.P메시지_에러)
	공용.F테스트_같음(테스트, 회신.G길이(), 0)
	
	회신 = 공용.New질의(공용.P메시지_GET, 종목_모음[0], 공용.P포지션_롱, "숫자 아님").G회신(Ch종목)
	공용.F테스트_에러발생(테스트, 회신.G에러())
	공용.F테스트_같음(테스트, 회신.G구분(), 공용.P메시지_에러)
	공용.F테스트_같음(테스트, 회신.G길이(), 0)
	*/
