...
func TestF가격정보_Go루틴_수신_및_캐시(테스트 *testing.T) {
	...
	for i:=0 ; i < 값_수량 ; i++ {
		...
		// 값 쓰기
		회신 := 공용.New질의(공용.P메시지_SET, 종목코드, 통화.G단위(), 통화.G실수값(), 
					가격정보.G시점().Format(공용.P시간_형식) ).G회신(Ch가격정보, 공용.P타임아웃_Go)
...
func TestF가격정보_Go루틴_구독채널_등록(테스트 *testing.T) {
	...
	for i:=0 ; i < 가격정보_수량 ; i++ {
		...
		회신 := 공용.New질의(공용.P메시지_SET, 종목코드, 
					통화단위, 금액, time.Now()).G회신(Ch가격정보, 공용.P타임아웃_Go)
		공용.F테스트_에러없음(테스트, 회신.G에러())
		공용.F테스트_같음(테스트, 회신.G길이(), 0)
	}
...
func TestF가격정보_Go루틴_Python(테스트 *testing.T) {
	공용.F메모("Python 테스트")
}
...
func f가격정보_Go루틴_테스트_초기화() error {
	...
	회신 := 공용.New질의(공용.P메시지_초기화).G회신(ch가격정보_Go루틴_제어, 공용.P타임아웃_Go)
