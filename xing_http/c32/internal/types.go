...
func (s *S콜백_대기_항목) G회신값() interface{} {
...
func New콜백_저장소() *S콜백_저장소 {
	s := new(S콜백_저장소)
	s.저장소 = make(map[int]*S콜백_대기_항목)
	s.최근_정리_시간 = time.Now()
...
func (s *S콜백_저장소) G값(식별번호 int) *S콜백_대기_항목 {
	s.s정리()
...
func (s *S콜백_저장소) S추가(식별번호 int, TR코드 string) chan interface{} {
	s.s정리()
