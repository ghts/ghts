...
//export InitGoDLL
func InitGoDLL(_hWnd unsafe.Pointer) (반환값 bool) {
	defer lib.S예외처리{M함수: func() { 반환값 = false }}.S실행()

	k32.F체크("InitGoDLL() called.")
	k32.F윈도우_핸들_설정(w32.HWND(_hWnd))
	k32.F체크("InitGoDLL() OK.")
...
//export NotifyLoginResult
func NotifyLoginResult(로그인_여부 bool) {
	k32.F체크(lib.F조건부_문자열(로그인_여부, "Login Success.", "Login Failure."))

	k32.Ch로그인 <- 로그인_여부

	k32.F체크("Login result submitted.")
}
