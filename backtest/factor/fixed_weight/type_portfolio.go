...
func (s *S포트폴리오) s주문(종목코드 string, 매도_매수_구분 lib.T매도_매수_구분, 수량 int64, 가격 float64, 매매_상세_구분 T매매_상세_구분) {
	if 매도_매수_구분 != lib.P매도 && 매도_매수_구분 != lib.P매수 {
	...
	} else if 보유_수량, 존재함 := s.M보유_현황.M주식[종목코드]; 매도_매수_구분 == lib.P매도 && (!존재함 || 보유_수량 <= 0) {
		panic(lib.New에러("미보유 종목 매도 : '%v' '%v' '%v' '%v'", s.M일자, s.M가격구분, 종목코드, 매매_상세_구분))
	} else if 매수_금액, 현금 := float64(수량)*가격, s.M보유_현황.M현금; 매도_매수_구분 == lib.P매수 && 매수_금액 > 현금 {
		panic(lib.New에러("보유 현금 초과 매수 : '%v' '%v' '%v' '%v' '%v' '%v'", s.M일자, s.M가격구분, 종목코드, 매매_상세_구분, 매수_금액, 현금))
	}
