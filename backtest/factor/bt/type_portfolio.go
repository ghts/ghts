...
func (s *S포트폴리오) G리밸런싱_기준_가격(종목코드 string) float64 {
	...
	return s.M데이터_처리기.(I데이터_처리기_백테스트_전용).G가격(일자, P종가, 종목코드) // 리밸런싱은 종가 기준으로 진행했음.
}
...
func (s *S포트폴리오) S리밸런싱_실행(종목코드_모음 []string) error {
	...
	for _, 종목코드 := range 종목코드_모음 {
		현재가 := s.M데이터_처리기.G현재가(종목코드)
		수량 := int64(종목당_금액 / 현재가)

		매수_주문 := new(S매매)
		매수_주문.M종목코드 = 종목코드
		매수_주문.M일자 = s.M일자
		매수_주문.M가격 = s.M데이터_처리기.G현재가(종목코드)
		매수_주문.M매도_매수_구분 = lib.P매수
		매수_주문.M수량 = 수량
		매수_주문.M매매_상세_구분 = P리밸런싱_매수
...
func (s *S포트폴리오) S부분_익절(종목코드 string, 비율 float64) {
	if 보유_수량, 존재함 := s.M보유_현황.M주식[종목코드]; !존재함 {
		...
		매도_주문.M종목코드 = 종목코드
		매도_주문.M일자 = s.M일자
		매도_주문.M가격 = s.M데이터_처리기.G현재가(종목코드)
		매도_주문.M매도_매수_구분 = lib.P매도
		매도_주문.M수량 = lib.F중간값(int64(매도_수량), 0, 보유_수량) // 상한(보유_수량)/하한(0)를 벗어나지 않도록 중간값.
		매도_주문.M매매_상세_구분 = P부분_익절

		s.S주문_체결(매도_주문)
	}
}

func (s *S포트폴리오) s주식_매도(종목코드 string, 매매_상세_구분 T매매_상세_구분) {
	if 수량, 존재함 := s.M보유_현황.M주식[종목코드]; !존재함 {
		panic(lib.New에러("매도 종목 미보유 : '%v' '%v' '%v' '%v'", s.M일자, s.M가격구분, 종목코드, 매매_상세_구분))
	} else if 수량 <= 0 {
		panic(lib.New에러("매도 종목 수량 오류 : '%v' '%v' '%v' '%v' '%v'", s.M일자, s.M가격구분, 종목코드, 매매_상세_구분, 수량))
	} else {
		매도_주문 := new(S매매)
		매도_주문.M종목코드 = 종목코드
		매도_주문.M일자 = s.M일자
		...
		매도_주문.M매도_매수_구분 = lib.P매도
		매도_주문.M수량 = 수량
		매도_주문.M매매_상세_구분 = 매매_상세_구분

		s.S주문_체결(매도_주문)
	}
}

func (s *S포트폴리오) s주식_전량_매도(매매_상세_구분 T매매_상세_구분) {
	for 종목코드 := range s.M보유_현황.M주식 {
		s.s주식_매도(종목코드, 매매_상세_구분)
	}
}
...
func (s *S일자별_자산_현황) G평가액(가격구분 T가격_구분, 데이터_처리기 I데이터_처리기) (평가액 float64) {
