...
func F일일_가격정보_수집(db *sql.DB, 종목코드_모음 []string) (에러 error) {
	...
	//당일_데이터만_검색할_종목코드 := make([]string, 0)
	...
	for i, 종목코드 := range 종목코드_모음 {
		...
		if 시작일.After(xing.F당일()) {
			lib.F문자열_출력("%v [%v] : 이미 최신 데이터로 업데이트 되어 있음.\n", i, 종목코드)
			continue
...
//func f당일_가격정보_수집(db *sql.DB, 종목코드_모음 []string) (에러 error) {
//	if !lib.F지금().After(xing.F당일().Add(18 * lib.P1시간)) {
//		return lib.New에러("폐장 이후에만 데이터 수집 가능합니다.")
//	}
//
//	종목코드_모음_나머지 := 종목코드_모음
//
...
//		}
//
//		for _, 일일_데이터 := range 응답값_맵 {
//			일일_가격정보 := lib.New일일_가격정보(
//				일일_데이터.M종목코드,
//				xing.F당일(),
//				일일_데이터.M시가,
//				일일_데이터.M고가,
...
//				일일_데이터.M누적_거래량)
//
//			일일_가격정보_슬라이스 := []*lib.S일일_가격정보{일일_가격정보}
//
//			종목별_일일_가격정보_모음, 에러 := lib.New종목별_일일_가격정보_모음(일일_가격정보_슬라이스)
//			if 에러 != nil {
//				lib.F에러_출력(에러)
