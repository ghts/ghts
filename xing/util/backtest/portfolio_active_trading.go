...
func New모의_액티브_매매_포트폴리오(초기_자본 float64, 거래당_최대_손실비율_퍼센트 float64, 최대_동시_진행_거래_수량 int) *S액티브_매매_포트폴리오 {
	...
	s.M매수_신호_처리기 = func(매수_신호 I매수_신호) {
		...
		if 매수_신호.G손절폭() > 0 {
			...
			수량 = int64(s.M자본 / trade.F모의_매수_거래가(매수_신호.G기준가(), 가상_슬리피지_비용))
		}
		...
		매매.M손절폭 = 매수_신호.G손절폭()
		매매.M매수_일자 = 매수_신호.G일자()
		매매.M매수_가격 = trade.F모의_매수_거래가(매수_신호.G기준가(), 가상_슬리피지_비용) // API 호출 없는 모의 거래.
	...
	s.M매도_신호_처리기 = func(매도_신호 I매도_신호) {
		for i := 0; i < len(s.M진행_중_매매); i++ {
			...
			if 매매.G종목코드() == 매도_신호.G종목코드() {
				...
				// API 호출 없는 모의 거래.
				매매.S매도_가격(trade.F모의_매도_거래가(매도_신호.G기준가(), 가상_슬리피지_비용))
	...
	s.M손절매_처리기 = func(데이터 I손절매_확인_정보) {
		for i := 0; i < len(s.M진행_중_매매); i++ {
			...
			if 매매.G손절매_필요(데이터.G종목코드(), 데이터.G기준가()) {
				...
				// API 호출 없는 모의 거래.
				매매.S매도_가격(trade.F모의_매도_거래가(데이터.G기준가(), 가상_슬리피지_비용))
