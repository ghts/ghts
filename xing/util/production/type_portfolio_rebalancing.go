...
func (s *S리밸런싱_포트폴리오) G종목코드_모음() []string {
	종목코드_모음 := make([]string, len(s.M종목별_데이터_모음))

	for i, 종목별_데이터 := range s.M종목별_데이터_모음 {
		종목코드_모음[i] = 종목별_데이터.G종목코드()
	}
...
func (s *S리밸런싱_포트폴리오) S현황_업데이트() (주문_수량_합계 int64, 에러 error) {
	defer lib.S예외처리{M에러: &에러, M함수: func() { 주문_수량_합계 = 0 }}.S실행()
	...
	주문_가능_금액 := float64(응답CSPAQ12200.M증거금률100퍼센트주문가능금액) * 0.99 // 1% 여유를 둔다.
	...
	for _, 값 := range s.M종목별_데이터_모음 {
		...
		case 값.G현재_평가액() > 값.G기준_금액():
			...
			if 값.G주문_수량() > 0 && 값.G주문_수량() < 값.G현재_수량() {
				값.S주문_수량(값.G주문_수량() + 1)
			}
		...
		case 값.G현재_평가액() < 값.G기준_금액():
			값.S주문_수량(int64((값.G기준_금액()-값.G현재_평가액())/값.G현재가()) - 1)
			...
			for {
				종목별_주문_금액 := float64(값.G주문_수량()) * 값.G현재가()

				if 종목별_주문_금액 > 주문_가능_금액 {
					값.S주문_수량(값.G주문_수량() - 1)
				} else {
					주문_가능_금액 -= 종목별_주문_금액
					break
	...
	for _, 값 := range s.M종목별_데이터_모음 {
		주문_수량_합계 += lib.F절대값_정수64(값.G주문_수량())
	}
...
func (s *S리밸런싱_포트폴리오) S주문_실행() {
	현재가_정보_맵, 에러 := xing.TrT8407_현물_멀티_현재가_조회(s.G종목코드_모음())
	lib.F확인(에러)
	...
	for _, 종목별_데이터 := range s.M종목별_데이터_모음 {
		...
		case lib.F한국증시_정규_거래_시간임():
			if 현재가_정보, 존재함 := 현재가_정보_맵[질의값_주문.M종목코드]; 존재함 {
				질의값_주문.M호가유형 = lib.P호가_지정가

				switch 종목별_데이터.G매수_매도_구분() {
				case lib.P매도:
					질의값_주문.M주문단가 = 현재가_정보.M매도호가
				case lib.P매수:
					질의값_주문.M주문단가 = 현재가_정보.M매수호가
				default:
					panic(lib.New에러("예상하지 못한 경우 : %v", 종목별_데이터.G매수_매도_구분()))
				}
			} else {
				질의값_주문.M주문단가 = 0
