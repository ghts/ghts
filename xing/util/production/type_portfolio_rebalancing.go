...
func (s *S종목별_공통_데이터) G회당_주문_수량_상한선() int64 {
...
func (s *S리밸런싱_포트폴리오) S주문금액_낮은_순서대로_정렬() {
	sort.Sort(s)
}
func (s *S리밸런싱_포트폴리오) Len() int {
	return len(s.M종목별_데이터_모음)
}
func (s *S리밸런싱_포트폴리오) Swap(i, j int) {
	s.M종목별_데이터_모음[i], s.M종목별_데이터_모음[j] = s.M종목별_데이터_모음[j], s.M종목별_데이터_모음[i]
}
func (s *S리밸런싱_포트폴리오) Less(i, j int) bool {
...
func (s *S리밸런싱_포트폴리오) S현황_업데이트() {
	...
	for _, 종목_현황 := range 응답CSPAQ12300.M반복값_모음 {
		...
		lib.F조건부_실행(s.F현황_업데이트_추가_작업 != nil, s.F현황_업데이트_추가_작업, 종목별_데이터, 종목_현황)
	...
	for _, 값 := range s.M종목별_데이터_모음 {
		...
		case 값.G현재_평가액() > 값.G기준_금액():
			...
			if 값.G회당_주문_수량_상한선() > 0 &&
				값.G주문_수량() > 값.G회당_주문_수량_상한선() {
				값.S주문_수량(값.G회당_주문_수량_상한선())
			}
		case 값.G현재_평가액() < 값.G기준_금액():
...
func (s *S리밸런싱_포트폴리오) s주문_도우미(매도_매수_구분 lib.T매도_매수_구분) {
