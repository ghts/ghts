...
func TestF체결_실시간_정보(t *testing.T) {
	...
	lb.F테스트_에러없음(t, 에러)
	lb.F테스트_같음(t, 종목_코스피.G시장구분(), lb.P시장구분_코스피)

	종목_코스닥, 에러 := F종목by코드(종목코드_코스닥)
	lb.F테스트_에러없음(t, 에러)
	lb.F테스트_같음(t, 종목_코스닥.G시장구분(), lb.P시장구분_코스닥)

	종목_ETF, 에러 := F종목by코드(종목코드_ETF)
	lb.F테스트_에러없음(t, 에러)
	lb.F테스트_같음(t, 종목_ETF.G시장구분(), lb.P시장구분_ETF)

	소켓SUB_실시간 := lb.F확인2(nano.NewNano소켓SUB(xt.F주소_실시간()))
	lb.F테스트_에러없음(t, F체결_실시간_정보_구독(종목코드_코스피))
	lb.F테스트_에러없음(t, F체결_실시간_정보_구독(종목코드_코스닥))
	lb.F테스트_에러없음(t, F체결_실시간_정보_구독(종목코드_ETF))

	defer func() {
		lb.F테스트_에러없음(t, F체결_실시간_정보_해지(종목코드_코스피))
		lb.F테스트_에러없음(t, F체결_실시간_정보_해지(종목코드_코스닥))
		lb.F테스트_에러없음(t, F체결_실시간_정보_해지(종목코드_ETF))
	}()
	...
	for i := 0; i < 1000; i++ {
		...
		lb.F테스트_참임(t, 값.M고가 >= 값.M가중평균가)
		lb.F테스트_참임(t, 값.M저가 <= 값.M시가)
		lb.F테스트_참임(t, 값.M저가 <= 값.M현재가)
		lb.F테스트_참임(t, 값.M저가 <= 값.M가중평균가)
		lb.F테스트_참임(t, 값.M체결량 > 0)
		lb.F테스트_참임(t, 값.M누적거래량 >= 값.M체결량)
		lb.F테스트_참임(t, 값.M누적거래대금 > 0)
		lb.F테스트_참임(t, 값.M매도호가 > 값.M매수호가 || 값.M매도호가 == 0)
		lb.F테스트_같음(t, 값.M거래소_구분, xt.P거래소_KRX, xt.P거래소_NXT, xt.P거래소_통합)
