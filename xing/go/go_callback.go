...
func Go루틴_관리(ch초기화 chan lb.T신호) (에러 error) {
	...
	ch도우미_초기화 := make(chan lb.T신호, V콜백_도우미_수량)
...
func go루틴_콜백_처리_도우미(ch초기화 chan lb.T신호, ch도우미_종료 chan error) (에러 error) {
	if lb.F공통_종료_채널_닫힘() {
		return
	}
	...
	for {
		if lb.F공통_종료_채널_닫힘() {
			...
			lb.F에러_출력(에러)
		} else if 콜백값, ok := i값.(lb.I콜백); !ok {
			panic(lb.New에러("'I콜백'형이 아님 : '%T'", i값))
		} else {
			변환_형식 := 바이트_변환_모음.G변환_형식(0)

			switch 콜백값.G콜백() {
			case lb.P콜백_TR데이터, lb.P콜백_메시지_및_에러, lb.P콜백_TR완료, lb.P콜백_타임아웃:
				lb.F확인1(f콜백_TR데이터_처리기(콜백값))
			case lb.P콜백_신호:
				if 에러 = f콜백_신호_처리기(콜백값); 에러 != nil {
					lb.F에러_출력(에러)
				}
			case lb.P콜백_링크_데이터, lb.P콜백_실시간_차트_데이터:
				panic("TODO") // 변환값 := 값.(*S콜백_기본형)
			case lb.P콜백_소켓_테스트:
				// 아무 것도 안하고 OK 응답 보냄.
			default:
				panic(lb.New에러("예상하지 못한 콜백 구분값 : '%v'", 콜백값.G콜백()))
			}

			ctx.S송신(변환_형식, lb.P신호_OK)
		}
	}
}
