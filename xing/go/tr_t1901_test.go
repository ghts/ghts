...
func TestT1901_ETF_시세_조회(t *testing.T) {
	...
	//s.M업종_전일대비등락폭 = lib.F2실수_소숫점_추가_단순형(g.Upchange, 2)
	//s.M업종_전일대비등락율 = lib.F2실수_소숫점_추가_단순형(g.Updiff, 2)
	...
	//s.NAV_전일대비등락폭 = lib.F2실수_소숫점_추가_단순형(g.Navchange, 2)
	//s.NAV_전일대비등락율 = lib.F2실수_소숫점_추가_단순형(g.Navdiff, 2)
	...
	//s.ETN만기일 = lib.F2포맷된_일자_단순형("20060102", g.Lastymd)
	//s.ETN지급일 = lib.F2포맷된_일자_단순형("20060102", g.Payday)
	//s.ETN최종거래일 = lib.F2포맷된_일자_단순형("20060102", g.Lastdate)
	//s.ETN발행시장참가자 = lib.F2문자열(g.Issuernmk)
	//s.ETN만기상환가격결정_시작일 = lib.F2포맷된_일자_단순형("20060102", g.Last_sdate)
	//s.ETN만기상환가격결정_종료일 = lib.F2포맷된_일자_단순형("20060102", g.Last_edate)
