...
func TestCSPAQ12300_현물계좌_잔고내역_조회(t *testing.T) {
	...
	for 종목코드, 값 := range 값_모음 {
		...
		lb.F테스트_참임(t, lb.F오차율_퍼센트(평가손익, float64(값.M평가손익)) < 10, lb.F오차율_퍼센트(평가손익, float64(값.M평가손익)))
		if lb.F오차율_퍼센트(평가손익, float64(값.M평가손익)) > 3.0 {
			lb.F문자열_출력("평가손익 기대값 대비 오차율 큼 : %v%%, %v, %v",
				int64(lb.F오차율_퍼센트(평가손익, float64(값.M평가손익))), int64(평가손익), 값.M평가손익)
		}
