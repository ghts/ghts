...
func TestCSPAQ12300_현물계좌_잔고내역_조회(t *testing.T) {
	...
	응답값, 에러 := TrCSPAQ12300_현물계좌_잔고내역_조회(계좌번호, xt.CSPAQ12300_평균_단가, false)
	lib.F테스트_에러없음(t, 에러)
	...
	lib.F테스트_같음(t, 값1.M잔고생성_구분, "0")    // 0:전체, 1:현물
	lib.F테스트_같음(t, 값1.M수수료적용_구분, "0")   // 0:수수료 미적용, 1:수수료 적용
	lib.F테스트_같음(t, 값1.D2잔고기준조회_구분, "1") // 0:전부조회, 1:D2잔고 0이상만 조회
	lib.F테스트_같음(t, 값1.M단가_구분, "0")      // 0:평균단가, 1:BEP단가
