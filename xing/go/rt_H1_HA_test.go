...
func TestF호가_잔량_실시간_정보_구독_해지(t *testing.T) {
	t.Parallel()
	...
	종목_코스닥, 에러 := F종목by코드(종목코드_코스피)
	lib.F테스트_에러없음(t, 에러)
	lib.F테스트_같음(t, 종목_코스닥.G시장구분(), lib.P시장구분_코스닥)

	종목_ETF, 에러 := F종목by코드(종목코드_코스피)
	lib.F테스트_에러없음(t, 에러)
	...
	for i:=0 ; i<1000 ; i++ {
		...
		i실시간_정보 := 바이트_변환_모음.S해석기(xt.F바이트_변환값_해석).G해석값_단순형(0)
		...
		lib.F체크포인트(i, 코스피_수신, 코스닥_수신, ETF_수신)
