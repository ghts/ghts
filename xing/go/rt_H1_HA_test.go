...
func TestF호가_잔량_실시간_정보(t *testing.T) {
	...
	lb.F테스트_에러없음(t, F호가_잔량_실시간_정보_구독(종목코드_코스피))
	lb.F테스트_에러없음(t, F호가_잔량_실시간_정보_구독(종목코드_코스닥))
	lb.F테스트_에러없음(t, F호가_잔량_실시간_정보_구독(종목코드_ETF))

	defer func() {
		lb.F테스트_에러없음(t, F호가_잔량_실시간_정보_해지(종목코드_코스피))
		lb.F테스트_에러없음(t, F호가_잔량_실시간_정보_해지(종목코드_코스닥))
		lb.F테스트_에러없음(t, F호가_잔량_실시간_정보_해지(종목코드_ETF))
	}()
	...
	for i := 0; i < 1000; i++ {
		...
		lb.F테스트_참임(t, 값.M중간가격 <= lb.F최소값(값.M매도호가_모음...))
		lb.F테스트_참임(t, 값.M중간가격 >= lb.F최대값(값.M매수호가_모음...))
		lb.F테스트_참임(t, 값.M매도중간가잔량합계수량 >= 0)
		lb.F테스트_참임(t, 값.M매수중간가잔량합계수량 >= 0)
		lb.F테스트_참임(t, 값.M중간가잔량합계수량 == 값.M매도중간가잔량합계수량+값.M매수중간가잔량합계수량)
		lb.F테스트_같음(t, 값.M중간가잔량구분, lb.P매도_매수_전체, lb.P매도, lb.P매수)
