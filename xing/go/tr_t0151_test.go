...
func TestT0151_현물_전일_매매일지_수수료(t *testing.T) {
	...
	응답값, 에러 := xt.TR_t0151_현물_전일_매매일지_수수료(계좌번호, F전일())
	lib.F테스트_에러없음(t, 에러)

	값 := 응답값.M헤더
	
	if 값.M매도_수량 > 0 {
		lib.F테스트_참임(t, 값.M매도_약정금액 > 0)
		lib.F테스트_참임(t, 값.M매도_수수료 > 0)
		lib.F테스트_참임(t, 값.M매도_거래세 >= 0)
		lib.F테스트_참임(t, 값.M매도_농특세 >= 0)
		lib.F테스트_참임(t, 값.M매도_제비용합 > 0)
		lib.F테스트_참임(t, 값.M매도_정산금액 > 0)
	}

	if 값.M매수_수량 > 0 {
		lib.F테스트_참임(t, 값.M매수_약정금액 > 0)
		lib.F테스트_참임(t, 값.M매수_수수료 > 0)
		lib.F테스트_참임(t, 값.M매수_제비용합 > 0)
		lib.F테스트_참임(t, 값.M매수_정산금액 > 0)
	}

	lib.F테스트_같음(t, 값.M합계_수량, 값.M매도_수량 + 값.M매수_수량)
	lib.F테스트_같음(t, 값.M합계_약정금액, 값.M매도_약정금액 + 값.M매수_약정금액)
	lib.F테스트_같음(t, 값.M합계_수수료, 값.M매도_수수료 + 값.M매수_수수료)
	lib.F테스트_같음(t, 값.M합계_거래세, 값.M매도_거래세)
	lib.F테스트_같음(t, 값.M합계_농특세, 값.M매도_농특세)
	lib.F테스트_같음(t, 값.M합계_제비용합, 값.M매도_제비용합 + 값.M매수_제비용합)
	lib.F테스트_같음(t, 값.M합계_정산금액, 값.M매도_정산금액 + 값.M매수_정산금액)
