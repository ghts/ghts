...
func TestT8411_F현물_차트_분틱(t *testing.T) {
	...
	for _, 값 := range 값_모음 {
		lb.F테스트_참임(t, lb.F2일자(값.M일자_시각).Equal(시작일자) || lb.F2일자(값.M일자_시각).After(시작일자), 값.M일자_시각)
		lb.F테스트_참임(t, lb.F2일자(값.M일자_시각).Equal(종료일자) || lb.F2일자(값.M일자_시각).Before(종료일자), 값.M일자_시각)
...
func F테스트_현물_차트_틱_응답_반복값_t8411(t *testing.T, 값 *xt.T8411_현물_차트_틱_응답_반복값, 종목코드 string) {
	...
	lb.F테스트_같음(t, 값.M시가, 값.M고가)
	lb.F테스트_같음(t, 값.M시가, 값.M저가)
	lb.F테스트_참임(t, 값.M시가 == 값.M종가, 값.M일자_시각, 값.M시가, 값.M종가)
