...
func F테스트_현물_차트_일주월_응답_반복값_t8413(t *testing.T, 값 *xt.T8413_현물_차트_일주월_응답_반복값, 종목코드 string) {
	...
	lib.F체크포인트(값.M시가, 값.M고가, 값.M저가, 값.M종가)
	lib.F테스트_참임(t, 값.M고가 >= 값.M시가, 값.M고가, 값.M시가)
	lib.F테스트_참임(t, 값.M고가 >= 값.M종가, 값.M고가, 값.M종가)
	lib.F테스트_참임(t, 값.M저가 <= 값.M시가, 값.M저가, 값.M시가)
	lib.F테스트_참임(t, 값.M저가 <= 값.M종가, 값.M저가, 값.M종가)
