...
func TrCSPAT00600_현물_정상주문(질의값 *xt.CSPAT00600_현물_정상_주문_질의값) (응답값 *xt.CSPAT00600_현물_정상_주문_응답, 에러 error) {
	...
	if 질의값.M호가유형 == lb.P호가_지정가 && 질의값.M주문단가 == 0 {
		...
		return nil, lb.New에러with출력("%v %v 잘못된 주문 수량 %v", 질의값.M계좌번호, 질의값.M종목코드, 질의값.M주문수량)
	...
	return F질의_단일TR[xt.CSPAT00600_현물_정상_주문_응답](질의값)
}
...
func TrCSPAT00800_현물_취소주문(질의값 *lb.S질의값_취소_주문) (응답값 *xt.CSPAT00800_현물_취소_주문_응답, 에러 error) {
	예외_처리 := lb.S예외처리{M에러: &에러, M에러_실행: func() { 응답값 = nil }}
	defer 예외_처리.S실행()
...
func TrCSPAQ12200_현물계좌_총평가(계좌번호 string) (응답값 *xt.CSPAQ12200_현물계좌_총평가_응답, 에러 error) {
	...
	return F질의_단일TR[xt.CSPAQ12200_현물계좌_총평가_응답](질의값)
}
...
func TrCSPAQ12300_현물계좌_잔고내역_조회(계좌번호 string, 단가_구분 xt.T단가_구분_CSPAQ12300,
	...
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.CSPAQ12300_현물계좌_잔고내역_응답](질의값))
...
func TrCSPAQ13700_현물계좌_주문체결내역(계좌번호 string, 주문일 time.Time,
	...
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.CSPAQ13700_현물계좌_주문체결내역_응답](질의값))
...
func TrCSPAQ22200_현물계좌_예수금_주문가능금액(계좌번호 string) (응답값 *xt.CSPAQ22200_현물계좌_예수금_주문가능금액_응답, 에러 error) {
	defer lb.S예외처리{M에러: &에러, M에러_실행: func() { 응답값 = nil }}.S실행()
	...
	return F질의_단일TR[xt.CSPAQ22200_현물계좌_예수금_주문가능금액_응답](질의값)
}

func TrT0150_현물_당일_매매일지(계좌번호 string) (응답값_모음 []*xt.T0150_현물_당일_매매일지_응답_반복값, 에러 error) {
	defer lb.S예외처리{M에러: &에러, M에러_실행: func() { 응답값_모음 = nil }}.S실행()
	...
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.T0150_현물_당일_매매일지_응답](질의값))
...
func TrT0151_현물_일자별_매매일지(계좌번호 string, 일자 time.Time) (응답값_모음 []*xt.T0151_현물_일자별_매매일지_응답_반복값, 에러 error) {
	defer lb.S예외처리{M에러: &에러, M에러_실행: func() { 응답값_모음 = make([]*xt.T0151_현물_일자별_매매일지_응답_반복값, 0) }}.S실행()
	...
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.T0151_현물_일자별_매매일지_응답](질의값))
...
func TrT0167_시각_조회() chan *xt.T0167_시각_조회_응답 {
	...
	go func(ch응답 chan *xt.T0167_시각_조회_응답) {
		...
		수신값 := lb.F확인2(F질의_단일TR[time.Time](질의값))
...
func TrT0425_현물_체결_미체결_조회(계좌번호, 종목코드 string, 체결_구분 lb.T체결_구분,
	...
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.T0425_현물_체결_미체결_조회_응답](질의값))
...
func TrT1101_현물_호가_조회(종목코드 string) (응답값 *xt.T1101_현물_호가_조회_응답, 에러 error) {
	...
	return F질의_단일TR[xt.T1101_현물_호가_조회_응답](질의값)
}

func TrT1102_현물_시세_조회(종목코드 string, 거래소_구분 ...xt.T거래소_구분) (응답값 *xt.T1102_현물_시세_조회_응답, 에러 error) {
	defer lb.S예외처리{M에러: &에러, M에러_실행: func() { 응답값 = nil }}.S실행()
	...
	return F질의_단일TR[xt.T1102_현물_시세_조회_응답](질의값)
}
...
반복문:
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.T1305_현물_기간별_조회_응답](질의값))

		거래소별_단축코드 = 수신값.M헤더.M거래소별단축코드
		연속키 = 수신값.M헤더.M연속키
		응답값_모음 = append(응답값_모음, 수신값.M반복값_모음.M배열...)

		lb.F조건부_패닉(수신값.M헤더.M수량 != int64(len(수신값.M반복값_모음.M배열)),
...
func TrT1310_현물_당일전일_분틱_조회(종목코드 string, 당일_전일_구분 xt.T당일_전일_구분, 분틱_구분 xt.T분틱_구분,
	...
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.T1310_현물_전일당일분틱조회_응답](질의값))
...
func TrT1404_관리종목_조회(시장_구분 lb.T시장구분, 관리_질의_구분 xt.T관리_질의_구분) (응답값_모음 []*xt.T1404_관리종목_조회_응답_반복값, 에러 error) {
	...
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.T1404_관리종목_조회_응답](질의값))
...
func TrT1405_투자경고_조회(시장_구분 lb.T시장구분, 투자경고_질의_구분 xt.T투자경고_질의_구분) (응답값_모음 []*xt.T1405_투자경고_조회_응답_반복값, 에러 error) {
	...
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.T1405_투자경고_조회_응답](질의값))

		응답값_모음 = append(응답값_모음, 수신값.M반복값_모음.M배열...)

		if 연속키 = 수신값.M헤더.M연속키; lb.F2문자열_공백_제거(연속키) == "" {
			break
		}
	}
...
func TrT1901_ETF_시세_조회(종목코드 string) (응답값 *xt.T1901_ETF_시세_조회_응답, 에러 error) {
	...
	return F질의_단일TR[xt.T1901_ETF_시세_조회_응답](질의값)
}
...
반복문:
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.T1902_ETF시간별_추이_응답](질의값))
		...
		} else if 수량_한도 > 0 && len(응답값_모음) >= 수량_한도 {
			break
...
func TrT1906_ETF_LP_호가_조회(종목코드 string) (응답값 *xt.T1906_ETF_LP_호가_조회_응답, 에러 error) {
	...
	return F질의_단일TR[xt.T1906_ETF_LP_호가_조회_응답](질의값)
}

// TrT3320_F기업정보_요약 : HTS 3302 화면. t3320은 참고자료로서 정보의 정확성이나 완전성은 보장하기는 어렵습니다.
func TrT3320_F기업정보_요약(종목코드 string) (응답값 *xt.T3320_기업정보_요약_응답, 에러 error) {
	...
	if 응답값, 에러 = F질의_단일TR[xt.T3320_기업정보_요약_응답](질의값); 에러 == nil {
		응답값.M종목코드 = 종목코드
	}
...
func TrT3341_재무_순위_종합(시장구분 lb.T시장구분, 재무순위_구분 xt.T재무순위_구분,
	...
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.T3341_재무순위_응답](질의값))
	...
	// 종목코드 없는 값 제거
	응답값_모음_최종 := make([]*xt.T3341_재무순위_응답_반복값, 0)

	for _, 응답값 := range 응답값_모음 {
		if lb.F2문자열_공백_제거(응답값.M종목코드) == "" {
			continue
		}

		응답값_모음_최종 = append(응답값_모음_최종, 응답값)
	}

	return 응답값_모음_최종, nil
}

func TrT8407_현물_멀티_현재가_조회_전종목() (현재가_맵 map[string]*xt.T8407_현물_멀티_현재가_조회_응답, 에러 error) {
	defer lb.S예외처리{M에러: &에러, M에러_실행: func() { 현재가_맵 = nil }}.S실행()
...
func TrT8407_현물_멀티_현재가_조회(종목코드_모음_전체 []string) (응답값_맵 map[string]*xt.T8407_현물_멀티_현재가_조회_응답, 에러 error) {
	defer lb.S예외처리{M에러: &에러, M에러_실행: func() { 응답값_맵 = nil }}.S실행()
	...
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.T8407_현물_멀티_현재가_조회_응답_모음](질의값))

		for _, 응답값 := range 수신값.M값_모음 {
			응답값_맵[응답값.M종목코드] = 응답값
		}
...
func TrT8410_현물_차트_일주월년(종목코드 string, 시작일, 종료일 time.Time, 주기구분 xt.T일주월년_구분, 수정_주가_여부 bool,
	...
	for {
		...
		질의값.M종목코드 = 종목코드
		질의값.M주기구분 = 주기구분
		질의값.M요청건수 = lb.F조건값(수량_제한 > 0 && 수량_제한 < xt.P압축_반복값_최대_수량, 수량_제한, xt.P압축_반복값_최대_수량)
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.T8410_현물_차트_일주월년_응답](질의값))
...
func TrT8411_현물_차트_틱(종목코드 string, 시작일자, 종료일자 time.Time, 추가_인수_모음 ...interface{}) (응답값_모음 []*xt.T8411_현물_차트_틱_응답_반복값, 에러 error) {
	...
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.T8411_현물_차트_틱_응답](질의값))
		...
		if 수량_제한 > 0 && len(응답값_모음) >= 수량_제한 {
			break
		} else if lb.F2문자열_공백_제거(연속일자) == "" || lb.F2문자열_공백_제거(연속시간) == "" {
			break
		}
...
func TrT8412_현물_차트_분(종목코드 string, 시작일자, 종료일자 time.Time, 주기 time.Duration, 추가_인수_모음 ...interface{}) (응답값_모음 []*xt.T8412_현물_차트_분_응답_반복값, 에러 error) {
	...
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.T8412_현물_차트_분_응답](질의값))
...
func TrT8428_증시주변자금추이(수량 int) (응답값_모음 []*xt.T8428_증시주변_자금추이_응답_반복값, 에러 error) {
	...
	for {
		...
		수신값 := lb.F확인2(F질의_단일TR[xt.T8428_증시주변_자금추이_응답](질의값))

		연속키 = 수신값.M헤더.M연속키
		응답값_모음 = append(응답값_모음, 수신값.M반복값_모음.M배열[1:]...) // 반복 조회 1번째 값은 중복이므로 제외.
...
func TrT8436_주식종목_조회(시장_구분 lb.T시장구분) (응답값_모음 []*xt.T8436_현물_종목조회_응답_반복값, 에러 error) {
	...
	수신값 := lb.F확인2(F질의_단일TR[xt.T8436_현물_종목조회_응답](질의값))
...
func F질의_단일TR[T응답값 any](질의값 lb.I질의값, 옵션_모음 ...interface{}) (응답값 *T응답값, 에러 error) {
	예외_처리 := lb.S예외처리{M에러: &에러, M에러_실행: func() { 응답값 = nil }}
	defer 예외_처리.S실행()
...
func F접속됨() (접속됨 bool, 에러 error) {
	defer lb.S예외처리{M에러: &에러, M에러_실행: func() { 접속됨 = false }}.S실행()
...
func F계좌_수량() (계좌_수량 int, 에러 error) {
	defer lb.S예외처리{M에러: &에러, M에러_실행: func() { 계좌_수량 = 0 }}.S실행()
...
func F계좌_번호(인덱스 int) (계좌_번호 string, 에러 error) {
	defer lb.S예외처리{M에러: &에러, M에러_실행: func() { 계좌_번호 = "" }}.S실행()

	회신_메시지 := F질의(lb.New질의값_정수(xt.TR계좌_번호, "", 인덱스))
...
func F계좌_이름(계좌_번호 string) (계좌_이름 string, 에러 error) {
	defer lb.S예외처리{M에러: &에러, M에러_실행: func() { 계좌_이름 = "" }}.S실행()
...
//	defer lb.S예외처리{M에러: &에러, M에러_실행: func() { 계좌_상세명 = "" }}.S실행()
//
...
//	defer lb.S예외처리{M에러: &에러, M에러_실행: func() { 계좌_별명 = "" }}.S실행()
//
...
//	defer lb.S예외처리{M에러: &에러, M에러_실행: func() { 서버_이름 = "" }}.S실행()
//
