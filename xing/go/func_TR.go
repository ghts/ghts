...
func TrCSPAQ12300_현물계좌_잔고내역_조회(계좌번호 string) (값 *xt.CSPAQ12300_현물계좌_잔고내역_응답, 에러 error) {
	...
	for {
		...
		질의값.M수수료적용_구분 = "1"   // 0:수수료 미적용, 1:수수료 적용
		질의값.D2잔고기준조회_구분 = "0" // 0:전부조회, 1:D2잔고 0이상만 조회
...
func TrCSPAQ13700_현물계좌_주문체결내역(계좌번호 string, 주문일 time.Time) (값 *xt.CSPAQ13700_현물계좌_주문체결내역_응답, 에러 error) {
	...
	값 = new(xt.CSPAQ13700_현물계좌_주문체결내역_응답)
	값.M반복값_모음 = make([]*xt.CSPAQ13700_현물계좌_주문체결내역_반복값, 0)
...
func TrT0150_현물_당일_매매일지_수수료(계좌번호 string) (응답값_모음 []*xt.T0150_현물_당일_매매일지_수수료_응답_반복값, 에러 error) {
	...
	응답값_모음 = make([]*xt.T0150_현물_당일_매매일지_수수료_응답_반복값, 0)

	for {
		질의값 := new(xt.T0150_현물_당일_매매일지_수수료_질의값)
		질의값.S질의값_기본형 = lib.New질의값_기본형(xt.TR조회, xt.TR현물_당일_매매일지_수수료_t0150)
		질의값.M계좌번호 = 계좌번호
		...
		값, ok := i응답값.(*xt.T0150_현물_당일_매매일지_수수료_응답)
		lib.F조건부_패닉(!ok, "예상하지 못한 자료형 : '%T'", i응답값)
	...
	응답값_모음_nil_포함 := 응답값_모음

	응답값_모음 = make([]*xt.T0150_현물_당일_매매일지_수수료_응답_반복값, 0)

	for _, 응답값 := range 응답값_모음_nil_포함 {
		if 응답값 == nil {
			continue
		}

		응답값_모음 = append(응답값_모음, 응답값)
	}

	return 응답값_모음, nil
}

func TrT0151_현물_전일_매매일지_수수료(계좌번호 string, 일자 time.Time ) (응답값_모음 []*xt.T0151_현물_전일_매매일지_수수료_응답_반복값, 에러 error) {
	defer lib.S예외처리{M에러: &에러, M함수: func() { 응답값_모음 = nil }}.S실행()
	...
	응답값_모음 = make([]*xt.T0151_현물_전일_매매일지_수수료_응답_반복값, 0)
	...
	for {
		...
		값, ok := i응답값.(*xt.T0151_현물_전일_매매일지_수수료_응답)
		lib.F조건부_패닉(!ok, "예상하지 못한 자료형 : '%T'", i응답값)
...
func TrT8432_지수선물_마스터_조회(구분 string) (응답값_모음 []*xt.T8432_지수선물_마스터_조회_반복값, 에러 error) {
	...
	lib.F조건부_패닉(!ok, "예상하지 못한 자료형 : '%T'", 응답값_모음)
