...
func TrT8407_현물_멀티_현재가_조회_전종목() (응답값_모음 []*xt.T8407_현물_멀티_현재가_조회_응답, 에러 error) {
	defer lib.S예외처리{M에러: &에러, M함수: func() { 응답값_모음 = nil }}.S실행()

	종목코드_모음_전체 := F종목코드_모음_전체()
	종목코드_모음_나머지 := 종목코드_모음_전체
	응답값_모음 = make([]*xt.T8407_현물_멀티_현재가_조회_응답, 0)
...
func TrT8407_현물_멀티_현재가_조회(종목코드_모음 []string) (응답값_맵 map[string]*xt.T8407_현물_멀티_현재가_조회_응답, 에러 error) {
	defer lib.S예외처리{M에러: &에러, M함수: func() { 응답값_맵 = nil }}.S실행()

	lib.F조건부_패닉(len(종목코드_모음) == 0, "조회할 종목코드가 존재하지 않습니다.")
