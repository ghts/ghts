...
//	질의값.S질의값_단일_종목 = lib.New질의값_단일_종목(xt.TR조회, xt.TR선물옵션_정상주문_CFOAT00100, 선옵_종목코드)
//	질의값.M계좌번호 = 계좌번호
//	질의값.M매매구분 = 매매구분
//	질의값.M호가유형 = 호가유형
//	질의값.M주문가격 = 주문가격
...
//	질의값.S질의값_단일_종목 = lib.New질의값_단일_종목_단순형()
...
func TrCSPAT00600_현물_정상주문(질의값 *xt.CSPAT00600_현물_정상_주문_질의값) (응답값 *xt.CSPAT00600_현물_정상_주문_응답, 에러 error) {
	...
	if 질의값.M호가유형 == lib.P호가_조건부_지정가 && f조건부_지정가_주문_불가(질의값.M종목코드) {
		질의값.M호가유형 = lib.P호가_지정가
	} else if 질의값.M호가유형 == lib.P호가_조건부_지정가 {
		최근_조건부_지정가_주문_종목코드 = 질의값.M종목코드 // 호가 적합성 오류가 발생할 경우를 대비한 기록.
	}

	i응답값 := lib.F확인2(F질의_단일TR(질의값))
...
func TrCSPAQ12300_현물계좌_잔고내역_조회(계좌번호 string, 단가_구분 xt.T단가_구분_CSPAQ12300,
	...
	for {
		...
		질의값.M수수료적용_구분 = lib.F조건값(수수료_적용_여부, "1", "0") // 0:수수료 미적용, 1:수수료 적용
...
func TrT1310_현물_당일전일_분틱_조회(종목코드 string, 당일전일_구분 xt.T당일전일_구분, 분틱_구분 xt.T분틱_구분,
	...
	defer func() {
		일자 := lib.F조건값(당일전일_구분 == xt.P당일전일구분_당일, F당일(), F전일())
		수량 = len(응답값_모음_역순)
...
func TrT1404_관리종목_조회(시장_구분 lib.T시장구분, 관리_질의_구분 xt.T관리_질의_구분) (응답값_모음 []*xt.T1404_관리종목_조회_응답_반복값, 에러 error) {
	...
	switch 시장_구분 {
	case lib.P시장구분_전체, lib.P시장구분_코스피, lib.P시장구분_코스닥:
		// OK
	default:
		return nil, lib.New에러("유효하지 않은 시장 구분값 (전체/코스피/코스닥 중 1개만 가능). %v", 시장_구분)
	}
	...
	case xt.P구분_관리, xt.P구분_불성실_공시, xt.P구분_투자_유의, xt.P구분_투자_환기:
		// OK
	default:
		return nil, lib.New에러("유효하지 않은 관리 질의 구분값 (관리/불성실 공시/투자 유지/투자 환기 중 1개만 가능). %v", 관리_질의_구분)
	}
...
func TrT1405_투자경고_조회(시장_구분 lib.T시장구분, 투자경고_질의_구분 xt.T투자경고_질의_구분) (응답값_모음 []*xt.T1405_투자경고_조회_응답_반복값, 에러 error) {
	...
	for {
		...
		if 연속키 = 값.M헤더.M연속키; lib.F2문자열_공백_제거(연속키) == "" {
			break
		}
	}
...
func TrT1717_종목별_매매주체_동향(종목코드 string, 일별_누적_구분 xt.T1717_일별_누적_구분, 시작일, 종료일 time.Time) (값_모음 []*xt.T1717_종목별_매매주체_동향_응답_반복값, 에러 error) {
	...
	질의값 := new(xt.T1717_종목별_매매주체_동향_질의값)
	질의값.S질의값_단일_종목 = lib.New질의값_단일_종목(xt.TR조회, xt.TR종목별_매매주체_동향_t1717, 종목코드)
	질의값.M일별_누적_구분 = 일별_누적_구분
	질의값.M시작일 = 시작일
	질의값.M종료일 = 종료일

	i응답값 := lib.F확인2(F질의_단일TR(질의값))
	응답값, ok := i응답값.(*xt.T1717_종목별_매매주체_동향_응답)
...
func TrT1901_ETF_시세_조회(종목코드 string) (응답값 *xt.T1901_ETF_시세_조회_응답, 에러 error) {
	...
	질의값 := lib.New질의값_단일_종목(xt.TR조회, xt.TR_ETF_시세_조회_t1901, 종목코드)
	i응답값 := lib.F확인2(F질의_단일TR(질의값))
	응답값, ok := i응답값.(*xt.T1901_ETF_시세_조회_응답)
	lib.F조건부_패닉(!ok, "TrT1901() 예상하지 못한 자료형 : '%T'", i응답값)
...
func TrT8407_현물_멀티_현재가_조회_전종목() (현재가_맵 map[string]*xt.T8407_현물_멀티_현재가_조회_응답, 에러 error) {
	...
	for {
		...
		응답값_맵 := lib.F확인2(TrT8407_현물_멀티_현재가_조회(종목코드_모음))
...
func TrT8407_현물_멀티_현재가_조회(종목코드_모음_전체 []string) (응답값_맵 map[string]*xt.T8407_현물_멀티_현재가_조회_응답, 에러 error) {
	...
	종목코드_모음_전체 = lib.F맵_키_모음(종목코드_맵) // 중복 및 오류 제거.
...
func TrT8410_현물_차트_일주월년(종목코드 string, 시작일, 종료일 time.Time, 주기구분 xt.T일주월년_구분, 수정_주가_여부 bool,
	...
	if 전송_제한_정보_모음.M맵[xt.TR현물_차트_일주월년_t8410].M10분당_전송_제한 > 0 {
		간격 = time.Duration(int64(lib.P10분) / int64(전송_제한_정보_모음.M맵[xt.TR현물_차트_일주월년_t8410].M10분당_전송_제한))
	}
...
func TrT8413_현물_차트_일주월(종목코드 string, 시작일, 종료일 time.Time, 주기구분 xt.T일주월년_구분,
	...
	if 전송_제한_정보_모음.M맵[xt.TR현물_차트_일주월_t8413].M10분당_전송_제한 > 0 {
		간격 = time.Duration(int64(lib.P10분) / int64(전송_제한_정보_모음.M맵[xt.TR현물_차트_일주월_t8413].M10분당_전송_제한))
	}
...
func TrT8428_증시주변자금추이(수량 int) (응답값_모음 []*xt.T8428_증시주변_자금추이_응답_반복값, 에러 error) {
	...
	if 수량 <= 0 {
		return nil, lib.New에러("잘못된 수량 : '%v'", 수량)
	}
	...
	for {
		조회_수량 := lib.F조건값(수량 < 누적_수량+200, 수량-누적_수량, 200)
		누적_수량 += 조회_수량
...
func F질의(질의값 lib.I질의값, 옵션_모음 ...interface{}) (값 *lib.S바이트_변환_모음) {
	...
	defer func() {
		...
		if 에러 != nil {
			...
			if 질의_에러_연속_발생_횟수.G값() > 10 {
				에러 = lib.New에러with출력("xing.F질의() 10회 연속 에러 발생. 즉시 종료.")
				os.Exit(0)
			}
		} else {
			질의_에러_연속_발생_횟수.S값(0)
	...
	return lib.F확인2(소켓REQ.G질의_응답(lib.P변환형식_기본값, 질의값))
}
...
func F질의_단일TR(질의값 lib.I질의값, 옵션_모음 ...interface{}) (값 interface{}, 에러 error) {
	...
	i식별번호 := lib.F확인2(F질의(질의값, 옵션_모음...).G해석값(0))
...
func F접속됨() (접속됨 bool, 에러 error) {
	...
	접속됨 = lib.F확인2(F질의(질의값, lib.P10초).G해석값(0)).(bool)
...
func F계좌_수량() (계좌_수량 int, 에러 error) {
	...
	계좌_수량 = lib.F확인2(회신_메시지.G해석값(0)).(int)
...
func F계좌_이름(계좌_번호 string) (계좌_이름 string, 에러 error) {
	...
	계좌_이름 = lib.F확인2(회신_메시지.G해석값(0)).(string)
