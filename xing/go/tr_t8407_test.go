...
func TestT8407_F현물_멀티_현재가_조회(t *testing.T) {
	...
	for _, 값 := range 값_모음 {
		...
		lib.F테스트_같음(t, lib.F오차(값.M현재가, 값.M전일_종가), 값.M전일종가대비등락폭)
		...
		lib.F테스트_참임(t, 값.M매도호가 > 0)
		lib.F테스트_참임(t, 값.M매수호가 > 0)
		lib.F테스트_참임(t, 값.M체결수량 >= 0)
		lib.F테스트_참임(t, 값.M체결강도 >= 0, 값.M종목코드, 값.M체결강도)
		lib.F테스트_참임(t, 값.M현재가 <= 값.M고가)
		lib.F테스트_참임(t, 값.M현재가 >= 값.M저가)
		lib.F테스트_참임(t, 값.M시가 <= 값.M고가)
		lib.F테스트_참임(t, 값.M시가 >= 값.M저가)
		lib.F테스트_참임(t, 값.M저가 >= 값.M하한가)
		lib.F테스트_참임(t, 값.M거래대금_백만 >= 0, 값.M종목코드, 값.M거래대금_백만)
		lib.F테스트_참임(t, 값.M우선_매도잔량 > 0)
		lib.F테스트_참임(t, 값.M우선_매수잔량 > 0)
		lib.F테스트_참임(t, 값.M총_매도잔량 > 0)
		lib.F테스트_참임(t, 값.M총_매수잔량 > 0)
		...
		case xt.P구분_보합:
			lib.F테스트_참임(t, lib.F오차(값.M현재가, 값.M전일_종가) <= 1000, 값.M현재가, 값.M전일_종가)
		case xt.P구분_하락:
			lib.F테스트_참임(t, 값.M현재가 < 값.M전일_종가, 값.M현재가, 값.M전일_종가)
		case xt.P구분_하한:
