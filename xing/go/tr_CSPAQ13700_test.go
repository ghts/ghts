...
func testCSPAQ13700_현물계좌_주문체결내역_도우미(t *testing.T, 계좌번호 string, 일자 time.Time) {
	응답값, 에러 := TrCSPAQ13700_현물계좌_주문체결내역(계좌번호, 일자)
	lib.F테스트_에러없음(t, 에러)

	//lib.F체크포인트(일자, len(응답값.M반복값_모음))

	값1 := 응답값.M헤더1
	lib.F테스트_같음(t, 값1.M레코드_수량, 1)
	lib.F테스트_같음(t, 값1.M계좌번호, 계좌번호)
	lib.F테스트_같음(t, 값1.M주문시장코드, "00", "10", "20")
	lib.F테스트_같음(t, 값1.M매매구분, "0", "1", "2")
	lib.F테스트_참임(t, 값1.M종목코드 == "", F종목코드_존재함(값1.M종목코드), 값1.M종목코드)
	lib.F테스트_같음(t, 값1.M체결여부, "0", "1", "3")
	lib.F테스트_같음(t, 값1.M주문일, 일자)
	lib.F테스트_같음(t, 값1.M연속키, 0, 1, 2, 3, 4, 5)
	lib.F테스트_같음(t, 값1.M역순구분, "0", "1")
	lib.F테스트_같음(t, 값1.M주문유형코드, "00", "98", "99")

	값2 := 응답값.M헤더2
	lib.F테스트_참임(t, 값2.M레코드_수량 > 0)
	lib.F테스트_참임(t, 값2.M매도체결금액 >= 0)
	lib.F테스트_참임(t, 값2.M매수체결금액 >= 0)
	lib.F테스트_참임(t, 값2.M매도체결수량 >= 0)
	lib.F테스트_참임(t, 값2.M매수체결수량 >= 0)
	lib.F테스트_참임(t, 값2.M매도주문수량 >= 0)
	lib.F테스트_참임(t, 값2.M매수주문수량 >= 0)

	if len(응답값.M반복값_모음) == 0 {
		return
	}
	...
	for _, 값 := range 응답값.M반복값_모음 {
		...
		lib.F테스트_같음(t, 값.M주문유형,
			...
			xt.P주문_상품매수, xt.P주문_선물대용매도_일반, xt.P주문_선물대용매도_반대,
			xt.P주문_현금매수_유가, xt.P주문_현금매수_정리, xt.P주문_장외매매)
