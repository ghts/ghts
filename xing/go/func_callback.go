...
func f콜백_TR데이터_처리기(값 lb.I콜백) (에러 error) {
	...
	if 대기_항목 == nil {
		switch 콜백 := 값.G콜백(); 콜백 {
		case lb.P콜백_TR데이터:
			panic(lb.New에러("식별번호 '%v' : nil 대기항목. %v", 식별번호, 값.G콜백()))
		case lb.P콜백_메시지_및_에러:
			if 변환값 := 값.(*lb.S콜백_메시지_및_에러); f에러_발생(TR코드, 변환값.M코드, 변환값.M내용) {
				panic(lb.New에러("식별번호 '%v' : nil 대기항목. 에러 발생. '%v' '%v' '%v'", 식별번호, TR코드, 변환값.M코드, 변환값.M내용))
			} else if 변환값.M코드 != "00000" {
				panic(lb.New에러("식별번호 '%v' : nil 대기항목. 메세지. '%v' '%v' '%v'", 식별번호, TR코드, 변환값.M코드, 변환값.M내용))
			} else if 변환값.M내용 != "조회완료" && 변환값.M내용 != "정상적으로 조회가 완료되었습니다." {
				panic(lb.New에러("식별번호 '%v' : nil 대기항목. 메세지. '%v' '%v' '%v'", 식별번호, TR코드, 변환값.M코드, 변환값.M내용))
			}
		default:
			panic(lb.F2문자열("예상하지 못한 경우 : '%v' '%v'", int(콜백), 콜백.String()))

		}
	}

	lb.F조건부_패닉(대기_항목 == nil, "식별번호 '%v' : nil 대기항목. %v", 식별번호, 값.G콜백())
	...
	switch 값.G콜백() {
	case lb.P콜백_TR데이터:
		if 에러 = f콜백_데이터_복원(대기_항목, 값.(*lb.S콜백_TR데이터).M데이터); 에러 != nil && 대기_항목.에러 == nil {
			switch {
		...
		f콜백_데이터_추가_설정(대기_항목, 값.(*lb.S콜백_TR데이터))
	case lb.P콜백_메시지_및_에러:
		변환값 := 값.(*lb.S콜백_메시지_및_에러)

		if f에러_발생(TR코드, 변환값.M코드, 변환값.M내용) {
			if strings.Contains(변환값.M내용, "호가적합성 오류(조건부지정가 주문이 불가한 종목)") &&
	...
	case lb.P콜백_TR완료:
		대기_항목.응답_완료 = true
	case lb.P콜백_타임아웃:
		대기_항목.에러 = lb.New에러with출력("타임아웃.")
	default:
		panic(lb.New에러with출력("예상하지 못한 경우. 콜백 구분값 : '%v', 자료형 : '%T'", 값.G콜백(), 값))
	}
...
func f콜백_데이터_식별번호(값 lb.I콜백) (식별번호 int, 대기_항목 *DLL32_콜백_대기_항목, TR코드 string) {
	switch 변환값 := 값.(type) {
	case *lb.S콜백_TR데이터:
		식별번호 = 변환값.M식별번호
	case *lb.S콜백_메시지_및_에러:
		식별번호 = 변환값.M식별번호
	case *lb.S콜백_정수값:
		식별번호 = 변환값.M정수값
	default:
		panic(lb.New에러("예상하지 못한 경우. 콜백 구분 : '%v', 자료형 : '%T'", 값.G콜백(), 값))
	}
...
func f콜백_데이터_복원(대기_항목 *DLL32_콜백_대기_항목, 수신값 *lb.S바이트_변환) error {
	...
	case
		...
		xt.TR현물_호가_조회_t1101, xt.TR현물_시세_조회_t1102, xt.TR_ETF_시세_조회_t1901,
		...
		대기_항목.대기값 = lb.F확인2(수신값.S해석기(xt.F바이트_변환값_해석).G해석값())
		대기_항목.데이터_수신 = true
	...
	default:
		return lb.New에러("구현되지 않은 TR코드. %v", 대기_항목.TR코드)
	}
...
func f콜백_데이터_추가_설정(대기_항목 *DLL32_콜백_대기_항목, 콜백_데이터 *lb.S콜백_TR데이터) {
	switch 대기_항목.TR코드 {
...
func f콜백_신호_처리기(콜백 lb.I콜백) (에러 error) {
	defer lb.S예외처리{M에러: &에러}.S실행()

	콜백_정수값, ok := 콜백.(*lb.S콜백_정수값)
	lb.F조건부_패닉(!ok, "f콜백_신호_처리기() 예상하지 못한 자료형 : '%T'", 콜백)

	정수값 := 콜백_정수값.M정수값
	신호 := lb.T신호_32비트_모듈(정수값)

	switch 신호 {
	case lb.P신호_DLL32_초기화:
		select {
		case ch신호_DLL32_초기화 <- 신호:
		default:
		}
	case lb.P신호_DLL32_LOGIN:
		select {
		case ch신호_DLL32_로그인 <- 신호:
		default:
		}
	case lb.P신호_DLL32_접속_끊김:
		lb.F문자열_출력("%v 접속 끊김 신호 수신", lb.F지금().Format("15:04"))
		F접속_끊김_설정()
		go F종료()
	case lb.P신호_DLL32_종료:
		lb.F문자열_출력("%v DLL32_종료 신호 수신", lb.F지금().Format("01/02 15:04"))
		select {
		case ch신호_DLL32_종료 <- 신호:
		default:
		}
	default:
		return lb.New에러with출력("%v 예상하지 못한 신호 : '%v'", lb.F지금().Format("15:04"), 신호)
	}
