		...
		if f에러_발생(TR코드, 변환값.M코드, 변환값.M내용) {
			if strings.Contains(변환값.M내용, "호가적합성 오류") &&
				...
				if 대기_항목.대기값 == nil {
					...
					lib.F체크포인트("호가적합성 오류 대기값 nil 1.", F종목_식별_문자열(종목코드))
				} else if 대기값, ok := 대기_항목.대기값.(*xt.CSPAT00600_현물_정상_주문_응답); !ok || 대기값 == nil {
					종목코드 = 최근_조건부_지정가_주문_종목코드
					lib.F체크포인트("호가적합성 오류 대기값 nil 2", F종목_식별_문자열(종목코드))
				} else if 대기값.M응답1 != nil && 대기값.M응답1.M종목코드 != "" {
					종목코드 = 대기값.M응답1.M종목코드
					lib.F체크포인트("호가적합성 오류 응답1", F종목_식별_문자열(종목코드))
				} else if 대기값.M응답2 != nil && 대기값.M응답2.M종목코드 != "" {
					종목코드 = 대기값.M응답2.M종목코드
					lib.F체크포인트("호가적합성 오류 응답2", F종목_식별_문자열(종목코드))
				} else {
					종목코드 = 최근_조건부_지정가_주문_종목코드
					lib.F체크포인트("호가적합성 오류 최근 종목", F종목_식별_문자열(종목코드))
				}
