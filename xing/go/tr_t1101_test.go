...
func TestT1101_현물_호가_조회(t *testing.T) {
	...
	lb.F테스트_참임(t, 값.KRX중간가격 >= 값.M저가 || 값.KRX중간가격 == 0, 값.KRX중간가격, 값.M저가)
	lb.F테스트_참임(t, 값.KRX중간가격 <= 값.M고가 || 값.KRX중간가격 == 0, 값.KRX중간가격, 값.M고가)
	lb.F테스트_참임(t, 값.KRX매도중간가잔량합계수량 >= 0)
	lb.F테스트_참임(t, 값.KRX매수중간가잔량합계수량 >= 0)
	lb.F테스트_참임(t, 값.KRX중간가잔량합계수량 >= 0)
	lb.F테스트_참임(t, 값.KRX중간가잔량합계수량 == 값.KRX매도중간가잔량합계수량+값.KRX매수중간가잔량합계수량)
	lb.F테스트_같음(t, 값.KRX중간가잔량구분, lb.P매도_매수_전체, lb.P매도, lb.P매수)
}
