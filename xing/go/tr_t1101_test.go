...
func TestT1101_현물_호가_조회(t *testing.T) {
	...
	lib.F테스트_같음(t, len(값.M매도잔량_모음), 10)
	lib.F테스트_같음(t, len(값.M매수잔량_모음), 10)
	...
	for i := 0; i < 10; i++ {
		...
		lib.F테스트_참임(t, 값.M매도잔량_모음[i] >= 0, 값.M매도잔량_모음[i])
		lib.F테스트_참임(t, 값.M매수잔량_모음[i] >= 0, 값.M매수잔량_모음[i])

		// (-) 값이 나오더라..
		//lib.F테스트_참임(t, 값.M직전매도대비수량_모음[i] >= 0, 값.M직전매도대비수량_모음[i])
		//lib.F테스트_참임(t, 값.M직전매수대비수량_모음[i] >= 0, 값.M직전매수대비수량_모음[i])

		매도호가수량합 = 매도호가수량합 + 값.M매도잔량_모음[i]
		매수호가수량합 = 매수호가수량합 + 값.M매수잔량_모음[i]
	}
