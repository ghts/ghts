...
func TestCFOAT00200_선물옵션_정정주문(t *testing.T) {
	...
	lib.F대기(lib.P500밀리초)
	정정가격 := 하한가 + 0.05 // 코스피 200 지수선물의 호가단위는 0.05
	정정수량 := 주문수량
	원주문번호1 := 응답값1.M응답2.M주문번호

	응답값2, 에러 := TrCFOAT00200_선물옵션_정정주문(종목코드, 계좌번호, 매매구분, 호가유형, 원주문번호1, 정정가격, 정정수량)
	lib.F테스트_에러없음(t, 에러)

	lib.F대기(lib.P500밀리초)
	원주문번호2 := 응답값2.M응답2.M주문번호
	취소수량 := 정정수량

	응답값3, 에러 := TrCFOAT00300_선물옵션_취소주문(종목코드, 계좌번호, 원주문번호2, 정정수량)
	lib.F테스트_에러없음(t, 에러)

	테스트_CFOAT00100_선물옵션_정상주문(t, 응답값1, 계좌번호, 종목코드, 매매구분, 호가유형, 주문가격, 주문수량)
	테스트_CFOAT00200_선물옵션_정정주문(t, 응답값2, 계좌번호, 종목코드, 매매구분, 호가유형, 정정가격, 원주문번호1, 정정수량)
	테스트_CFOAT00300_선물옵션_취소주문(t, 응답값3, 계좌번호, 종목코드, 원주문번호2, 취소수량)
}
...
func 테스트_CFOAT00200_선물옵션_정정주문(t *testing.T, 응답값 *xt.CFOAT00200_선물옵션_정정주문_응답,
	...
	lib.F테스트_참임(t,
		lib.F2일자(값1.M협의매매완료시각).Equal(time.Time{}) ||
			lib.F2일자(값1.M협의매매완료시각).Equal(lib.F금일()),
		값1.M협의매매완료시각)

	값2 := 응답값.M응답2
	lib.F테스트_참임(t, 값2.M주문번호 > 0, 값2.M주문번호)
	lib.F테스트_같음(t, 값2.M레코드갯수, 1) // 주문번호가 제대로 부여된 정상적인 경우에만 검사.
	lib.F테스트_다름(t, 값2.M지점명, "")
	lib.F테스트_다름(t, 값2.M계좌명, "")
	lib.F테스트_다름(t, 값2.M종목명, "")
	lib.F테스트_참임(t,
		(값2.M주문가능금액 == 0 && 값2.M현금주문가능금액 == 0 && 값2.M주문가능수량 == 0) ||
			(값2.M현금주문가능금액 > 0 && 값2.M주문증거금액 > 0 && 값2.M현금주문증거금액 > 0 && 값2.M주문가능수량 >= 0),
		값2.M현금주문가능금액, 값2.M주문증거금액, 값2.M현금주문증거금액, 값2.M주문가능수량)
}
