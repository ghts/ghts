...
func TestCFOAT00200_선물옵션_정정주문(t *testing.T) {
	...
	매매구분_모음 := []lib.T매도_매수_구분{lib.P매도매수_전체, lib.P매도, lib.P매수}
	...
	종목코드 := "??"
	매매구분 := 매매구분_모음[lib.F임의_범위_이내_정수값(0, 2)]
	호가유형 := 호가유형_모음[lib.F임의_범위_이내_정수값(0, 18)]
	주문가격 := float64(lib.F임의_범위_이내_실수64값(0, 3))
	정정수량 := int64(lib.F임의_범위_이내_정수값(0, 3))
	원주문번호 := int64(0)

	응답값, 에러 := TrCFOAT00200_선물옵션_정정주문(종목코드, 계좌번호, 매매구분, 호가유형, 원주문번호, 주문가격, 정정수량)
	lib.F테스트_에러없음(t, 에러)
	...
	lib.F테스트_참임(t, 값1.M정정수량 > 0, 값1.M정정수량)
