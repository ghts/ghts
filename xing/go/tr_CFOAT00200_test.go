...
func TestCFOAT00200_선물옵션_정정주문(t *testing.T) {
	...
	응답값2, 에러 := TrCFOAT00200_선물옵션_정정주문(종목코드, 계좌번호, 매매구분, 호가유형, 원주문번호, 정정가격, 정정수량)
	lib.F테스트_에러없음(t, 에러)
	테스트_CFOAT00200_선물옵션_정정주문(t, 응답값2, 계좌번호, 종목코드, 매매구분, 호가유형, 정정가격, 원주문번호, 정정수량)

	원주문번호 = 응답값2.M응답2.M주문번호
	취소수량 := 정정수량
	응답값3, 에러 := TrCFOAT00300_선물옵션_취소주문(종목코드, 계좌번호, 원주문번호, 정정수량)
	lib.F테스트_에러없음(t, 에러)
	테스트_CFOAT00300_선물옵션_취소주문(t, 응답값3, 계좌번호, 종목코드, 원주문번호, 취소수량)
}
