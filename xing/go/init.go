...
func f전일_당일_설정() (에러 error) {
	...
	수신값 := lb.F확인2(F질의_단일TR[xt.T1305_현물_기간별_조회_응답](질의값_기간별_조회))

	lb.F조건부_패닉(수신값.M헤더.M수량 != int64(수량), "예상하지 못한 수량 : '%v' '%v'", 수신값.M헤더.M수량, 수량)
	lb.F조건부_패닉(len(수신값.M반복값_모음.M배열) != 수량, "예상하지 못한 수량 : '%v' '%v'", len(수신값.M반복값_모음.M배열), 수량)
	lb.F조건부_패닉(수신값.M반복값_모음.M배열[0].M일자.Before(수신값.M반복값_모음.M배열[1].M일자), "예상하지 못한 순서")

	당일 := 수신값.M반복값_모음.M배열[0].M일자
	전일 := 수신값.M반복값_모음.M배열[1].M일자
	xt.F전일_당일_설정(전일, 당일)
...
func f전송_제한_초기화(TR코드_모음 []string) (에러 error) {
	...
	for TR코드, 전송_제한_정보 := range 전송_제한_정보_모음.M맵 {
		...
		if 전송_제한_정보.M10분당_전송_제한 > 0 {
			// 전송_제한_잠금(Lock)을 이미 가지고 있는 상태이므로 f10분당_전송_제한_읽기(RLock)를 호출하면
			// 같은 RWMutex 자기 자신 데드락이 발생함. 위의 1초 분기처럼 맵을 직접 접근.
			if 전송_권한, 존재함 := tr코드별_전송_제한_10분[TR코드]; 존재함 {
				...
				// G획득()은 잠금을 해제하지 않는 것이 설계이므로(해제는 S기록()에서 수행)
				// S기록()과 짝을 이루어 호출하지 않으면 다음 G획득()에서 데드락 발생.
