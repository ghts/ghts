...
func TestCFOFQ02400_선물옵션_미결제약정(t *testing.T) {
	...
	계좌번호 := lib.F확인(F계좌_번호(1)).(string)	// 옵션선물 계좌를 선택해야 함.
	...
	lib.F테스트_참임(t, 값2.M선물손익합계 >= 0)
	lib.F테스트_참임(t, 값2.M옵션손익합계 >= 0)
	lib.F테스트_참임(t, 값2.M전체손익합계 >= 0)

	for _, 반복값1 := range 응답값.M반복값1_모음 {
		lib.F테스트_같음(t, 반복값1.M선물옵션품목구분, xt.P선옵품목_코스피200_관련,
			xt.P선옵품목_코스피200_제외, xt.P선옵품목_코스닥50_관련)
	...
	for _, 반복값2 := range 응답값.M반복값2_모음 {
		lib.F체크포인트(반복값2.M종목코드)
		lib.F체크포인트(반복값2.M종목명)
		lib.F테스트_같음(t, 반복값2.M매도_매수_구분, lib.P매도매수_전체, lib.P매도, lib.P매수)
		...
		lib.F체크포인트(반복값2.M평가손익금액)
		lib.F체크포인트(반복값2.M평가수익률)
	}
}