...
func F로그인_정보_설정() (에러 error) {
	...
	if V로그인_정보 != nil {
		...
		return lib.New에러("로그인 ID값이 설정되어 있지 않습니다. 환경변수 '%v'에 설정하십시오.", P환경변수_로그인_ID)
	} else if 로그인_암호 := os.Getenv(P환경변수_로그인_암호); 로그인_암호 == "" {
		return lib.New에러("로그인 암호가 설정되어 있지 않습니다. 환경변수 '%v'에 설정하십시오.", P환경변수_로그인_암호)
	} else if 인증서_암호 := os.Getenv(P환경변수_인증서_암호); 인증서_암호 == "" {
		return lib.New에러("인증서 암호가 설정되어 있지 않습니다. 환경변수 '%v'에 설정하십시오.", P환경변수_인증서_암호)
	} else if 계좌_비밀번호 := os.Getenv(P환경변수_계좌_비밀번호); 계좌_비밀번호 == "" {
		return lib.New에러("계좌 암호가 설정되어 있지 않습니다. 환경변수 '%v'에 설정하십시오.", P환경변수_계좌_비밀번호)
	} else {
