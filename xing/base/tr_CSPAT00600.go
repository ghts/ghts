...
func (s *CSPAT00600_현물_정상_주문_질의값) String() string {
	if s.M주문단가 == 0 {
		return lb.F2문자열("%v %v %v %v주 %v", s.M코드, s.M계좌번호, s.M종목코드, s.M주문수량, s.M매도_매수_구분)
	} else {
		return lb.F2문자열("%v %v %v %v원 %v주 %v", s.M코드, s.M계좌번호, s.M종목코드, lb.F정수_쉼표_추가(s.M주문단가), s.M주문수량, s.M매도_매수_구분)
	}
}
...
func NewCSPAT00600InBlock(질의값 *CSPAT00600_현물_정상_주문_질의값, 비밀번호 string) (g *CSPAT00600InBlock1) {
	...
	lb.F바이트_복사_실수(g.OrdPrc[:], 질의값.M주문단가, 2)
	lb.F바이트_복사_문자열(g.BnsTpCode[:], lb.F2문자열(int(질의값.M매도_매수_구분)))
	lb.F바이트_복사_정수(g.OrdprcPtnCode[:], int(질의값.M호가유형.Xing코드()))
	lb.F바이트_복사_정수(g.MgntrnCode[:], int(질의값.M신용거래_구분))
	if 질의값.M신용거래_구분 == P신용거래_해당없음 {
		lb.F바이트_복사_문자열(g.LoanDt[:], "        ")
	} else {
		lb.F바이트_복사_문자열(g.LoanDt[:], 질의값.M대출일.Format("20060102"))
	}
	lb.F바이트_복사_정수(g.OrdCndiTpCode[:], int(질의값.M주문조건))
	lb.F바이트_복사_문자열(g.MbrNo[:], 질의값.M거래소_구분.String())
...
func NewCSPAT00600_현물_정상_주문_응답(b []byte) (값 *CSPAT00600_현물_정상_주문_응답, 에러 error) {
	...
	값.M응답1 = lb.F확인2(NewCSPAT00600_현물_정상_주문_응답1(b[:SizeCSPAT00600OutBlock1]))
	값.M응답2 = lb.F확인2(NewCSPAT00600_현물_정상_주문_응답2(b[SizeCSPAT00600OutBlock1:]))
...
func NewCSPAT00600_현물_정상_주문_응답1(b []byte) (s *CSPAT00600_현물_정상_주문_응답1, 에러 error) {
	...
	s.M주문가격 = lb.F확인2(lb.F2정수64(g.OrdPrc))
	s.M매도_매수_구분 = lb.T매도_매수_구분(lb.F확인2(lb.F2정수(g.BnsTpCode)))
	s.M호가유형 = F2호가유형(lb.F확인2(lb.F2정수_공백은_0(g.OrdprcPtnCode)))
	s.M프로그램_호가유형 = lb.F2문자열_공백_제거(g.PrgmOrdprcPtnCode)
	...
	s.M신용거래_구분 = T신용거래_구분(lb.F확인2(lb.F2정수(g.MgntrnCode)))
	s.M대출일 = lb.F2포맷된_일자_단순형_공백은_초기값("20060102", g.LoanDt)
	s.M회원번호 = lb.F2문자열_공백_제거(g.MbrNo)
	s.M주문조건_구분 = lb.T주문조건(lb.F확인2(lb.F2정수(g.OrdCndiTpCode)))
	...
	s.M운용지시_번호 = lb.F2문자열_공백_제거(g.OpDrtnNo)
	s.M유동성_공급자_여부 = lb.F문자열_비교(g.LpYn, "Y", true)
	s.M반대매매_구분 = lb.F2문자열_공백_제거(g.CvrgTpCode)
...
func NewCSPAT00600_현물_정상_주문_응답2(b []byte) (s *CSPAT00600_현물_정상_주문_응답2, 에러 error) {
	...
	if lb.F2문자열_공백_제거(g.OrdNo) == "" { // 주문 에러발생시 공백 문자열이 수신됨.
		return nil, lb.New에러("NewCSPAT00600_현물_정상_주문_응답2() : 주문번호 생성 에러.")
	}
	...
	if 시각_문자열 := lb.F2문자열_공백_제거(g.OrdTime); 시각_문자열 != "" {
		시각_문자열 = lb.F문자열_삽입(lb.F2문자열_공백_제거(g.OrdTime), ".", 6)
		s.M주문시각 = lb.F확인2(lb.F2금일_시각("150405.999999", 시각_문자열))
	} else {
		s.M주문시각 = time.Time{}
	}

	s.M주문시장_코드 = T주문시장구분(lb.F확인2(lb.F2정수(g.OrdMktCode)))
	...
	s.M재사용_주문수량 = lb.F확인2(lb.F2정수64(g.RuseOrdQty))
