...
func NewT8410InBlock(질의값 *T8410_현물_차트_일주월년_질의값) (g *T8410InBlock) {
	...
	lib.F바이트_복사_문자열(g.Sujung[:], lib.F조건값(질의값.M수정주가여부, "Y", "N"))
...
func NewT8410_현물_차트_일주월년_응답_헤더(b []byte) (값 *T8410_현물_차트_일주월년_응답_헤더, 에러 error) {
	...
	lib.F조건부_패닉(len(b) != SizeT8410OutBlock, "예상하지 못한 길이 : '%v", len(b))
	...
	값.M레코드_카운트 = lib.F확인2(lib.F2정수64(g.Rec_count))
	값.M정적VI상한가 = lib.F확인2(lib.F2정수64(g.Svi_uplmtprice))
	값.M정적VI하한가 = lib.F확인2(lib.F2정수64(g.Svi_dnlmtprice))
...
func NewT8410_현물_차트_일주월년_응답_반복값_모음(b []byte) (값 *T8410_현물_차트_일주월년_응답_반복값_모음, 에러 error) {
	...
	값.M배열 = make([]*T8410_현물_차트_일주월년_응답_반복값, 수량, 수량)
