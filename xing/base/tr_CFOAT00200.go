...
func NewCFOAT00200InBlock1(질의값 *CFOAT00200_선물옵션_정정주문_질의값, 비밀번호 string) (g *CFOAT00200InBlock1) {
	g = new(CFOAT00200InBlock1)
	...
	lb.F바이트_복사_정수(g.MdfyQty[:], 질의값.M정정수량)
...
func newCFOAT00200_선물옵션_정정주문_응답1(b []byte) (값 *CFOAT00200_선물옵션_정정주문_응답1, 에러 error) {
	defer lb.S예외처리{M에러: &에러, M함수: func() { 값 = nil }}.S실행()

	lb.F조건부_패닉(len(b) != SizeCFOAT00200OutBlock1, "예상하지 못한 길이 : '%v", len(b))
	...
	//값.M주문시장 = T주문시장구분(lb.F확인2(lb.F2정수(g.OrdMktCode))
	...
	값.M정정수량 = lb.F확인2(lb.F2정수64(g.MdfyQty)
	...
	//값.M관리사원번호 = lb.F2문자열(g.MgempNo)
	//값.M펀드ID = lb.F2문자열(g.FundId)
	//값.M펀드원주문번호 = lb.F확인2(lb.F2정수64(g.FundOrgOrdNo)
	//값.M펀드주문번호 = lb.F확인2(lb.F2정수64(g.FundOrdNo)

	return 값, nil
}

func newCFOAT00200_선물옵션_정정주문_응답2(b []byte) (값 *CFOAT00200_선물옵션_정정주문_응답2, 에러 error) {
	defer lb.S예외처리{M에러: &에러, M함수: func() { 값 = nil }}.S실행()

	lb.F조건부_패닉(len(b) != SizeCFOAT00200OutBlock2, "예상하지 못한 길이 : '%v", len(b))
