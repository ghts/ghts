...
func NewT8407_현물_멀티_현재가_조회_응답_반복값_모음(b []byte) (값_모음 []*T8407_현물_멀티_현재가_조회_응답, 에러 error) {
	...
	for i, g := range g_모음 {
		...
		s.M전일종가대비구분 = T전일대비_구분(lib.F확인2(lib.F2정수64(g.Sign)))
		s.M전일종가대비등락폭 = lib.F확인2(lib.F2정수64(g.Change))
		s.M전일종가대비등락율_퍼센트 = lib.F확인2(lib.F2실수_소숫점_추가(g.Diff, 2))
		s.M누적_거래량 = lib.F확인2(lib.F2정수64(g.Volume))
		s.M매도호가 = lib.F확인2(lib.F2정수64(g.Offerho))
		s.M매수호가 = lib.F확인2(lib.F2정수64(g.Bidho))
		...
		s.M우선_매도잔량 = lib.F확인2(lib.F2정수64(g.Offerrem))
		s.M우선_매수잔량 = lib.F확인2(lib.F2정수64(g.Bidrem))
		s.M총_매도잔량 = lib.F확인2(lib.F2정수64(g.Totofferrem))
		s.M총_매수잔량 = lib.F확인2(lib.F2정수64(g.Totbidrem))
		s.M전일_종가 = lib.F확인2(lib.F2정수64(g.Jnilclose))
