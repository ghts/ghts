...
func NewT0151_현물_일자별_매매일지_응답(b []byte) (값 *T0151_현물_일자별_매매일지_응답, 에러 error) {
	...
	lb.F조건부_패닉((len(b)-헤더_길이)%SizeT0151OutBlock1 != 0, "예상하지 못한 길이 : '%v'", len(b))
	값 = new(T0151_현물_일자별_매매일지_응답)

	값.M헤더 = lb.F확인2(NewT0151_현물_일자별_매매일지_응답_헤더(b[:SizeT0151OutBlock]))

	b = b[SizeT0151OutBlock+5:]

	값.M반복값_모음 = lb.F확인2(NewT0151_현물_일자별_매매일지_응답_반복값_모음(b))
