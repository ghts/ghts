...
func F고정_주소_설정(포트_번호 int) bool {
	주소_설정_잠금.Lock()
	defer 주소_설정_잠금.Unlock()

	주소_C32_호출 := lib.T주소(포트_번호)
	주소_C32_콜백 := lib.T주소(포트_번호 + 1)
	주소_실시간 := lib.T주소(포트_번호 + 2)
...
func F임의_주소_설정() {
	...
	for {
		if 임의_포트_번호 := lib.F임의_범위_이내_정수값(0, 30000); F고정_주소_설정(임의_포트_번호) {
			lib.F문자열_출력("포트_번호 : %v", 임의_포트_번호)
			return
		}
	}
}
