...
func NewCSPAQ12200_현물계좌_총평가_응답(b []byte) (값 *CSPAQ12200_현물계좌_총평가_응답, 에러 error) {
	...
	값.M출금가능금액 = lib.F확인2(lib.F2정수64(g2.MnyoutAbleAmt))
	값.M코스피_금액 = lib.F확인2(lib.F2정수64(g2.SeOrdAbleAmt))
	값.M코스닥_금액 = lib.F확인2(lib.F2정수64(g2.KdqOrdAbleAmt))
	값.M잔고평가금액 = lib.F확인2(lib.F2정수64(g2.BalEvalAmt))
	값.M미수금액 = lib.F확인2(lib.F2정수64(g2.RcvblAmt))
	값.M예탁자산총액 = lib.F확인2(lib.F2정수64(g2.DpsastTotamt))
	값.M손익율 = lib.F2실수_소숫점_추가_단순형_공백은_0(g2.PnlRat, 6)
	값.M투자원금 = lib.F확인2(lib.F2정수64(g2.InvstOrgAmt))
	값.M투자손익금액 = lib.F확인2(lib.F2정수64(g2.InvstPlAmt))
	값.M신용담보주문금액 = lib.F확인2(lib.F2정수64(g2.CrdtPldgOrdAmt))
	값.M예수금 = lib.F확인2(lib.F2정수64(g2.Dps))
	값.D1예수금 = lib.F확인2(lib.F2정수64(g2.D1Dps))
	값.D2예수금 = lib.F확인2(lib.F2정수64(g2.D2Dps))
	값.M대용금액 = lib.F확인2(lib.F2정수64(g2.SubstAmt))
	값.M현금미수금액 = lib.F확인2(lib.F2정수64(g2.MnyrclAmt))
	값.M증거금_현금 = lib.F확인2(lib.F2정수64(g2.MgnMny))
	값.M증거금_대용 = lib.F확인2(lib.F2정수64(g2.MgnSubst))
	값.M수표_금액 = lib.F확인2(lib.F2정수64(g2.ChckAmt))
	...
	값.D1추정인출가능금액 = lib.F확인2(lib.F2정수64(g2.D1PrsmptWthdwAbleAmt))
	값.D2추정인출가능금액 = lib.F확인2(lib.F2정수64(g2.D2PrsmptWthdwAbleAmt))
	값.M예탁담보대출금액 = lib.F확인2(lib.F2정수64(g2.DpspdgLoanAmt))
	...
	값.M원담보금액 = lib.F확인2(lib.F2정수64(g2.OrgPldgAmt))
	값.M부담보금액 = lib.F확인2(lib.F2정수64(g2.SubPldgAmt))
	값.M소요담보금액 = lib.F확인2(lib.F2정수64(g2.RqrdPldgAmt))
	값.M원담보부족금액 = lib.F확인2(lib.F2정수64(g2.OrgPdlckAmt))
	값.M담보부족금액 = lib.F확인2(lib.F2정수64(g2.PdlckAmt))
	값.M추가담보현금 = lib.F확인2(lib.F2정수64(g2.AddPldgMny))
	값.D1주문가능금액 = lib.F확인2(lib.F2정수64(g2.D1OrdAbleAmt))
	값.M신용이자미납금액 = lib.F확인2(lib.F2정수64(g2.CrdtIntdltAmt))
	값.M기타대여금액 = lib.F확인2(lib.F2정수64(g2.EtclndAmt))
	값.M익일추정반대매매금액 = lib.F확인2(lib.F2정수64(g2.NtdayPrsmptCvrgAmt))
	...
	값.M추가신용담보현금 = lib.F확인2(lib.F2정수64(g2.AddCrdtPldgMny))
	값.M신용담보재사용금액 = lib.F확인2(lib.F2정수64(g2.CrdtPldgRuseAmt))
	값.M추가신용담보대용 = lib.F확인2(lib.F2정수64(g2.AddCrdtPldgSubst))
