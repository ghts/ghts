...
func New현물_주문_접수(b []byte) (값 *S현물_주문_응답_실시간_정보, 에러 error) {
	...
	시각_문자열 := lb.F2문자열(g.Ordtm)
	시각_문자열 = 시각_문자열[:6] + "." + 시각_문자열[7:]
	시각 := lb.F확인2(lb.F2금일_시각("150405.999", 시각_문자열))
	종목코드 := trade.F종목코드_보정(lb.F2문자열_공백_제거(g.Shtcode))
	...
	값.M종목코드 = 종목코드
	값.M수량 = lb.F확인2(lb.F2정수64(g.Ordqty))
	값.M가격 = lb.F확인2(lb.F2정수64(g.Ordprice))
	값.M잔량 = 0
	값.M시각 = 시각
...
func New현물_주문_체결(b []byte) (값 *S현물_주문_응답_실시간_정보, 에러 error) {
	...
	시각_문자열 := lb.F2문자열(g.Exectime)
	시각_문자열 = 시각_문자열[:6] + "." + 시각_문자열[7:]
	시각 := lb.F확인2(lb.F2금일_시각("150405.999", 시각_문자열))
	종목코드 := trade.F종목코드_보정(lb.F2문자열_공백_제거(g.ShtnIsuno))
	...
	값.M응답_구분 = F2주문_응답_구분(g.Trcode)
	값.M종목코드 = 종목코드
	값.M수량 = lb.F확인2(lb.F2정수64(g.Execqty))
	값.M가격 = lb.F확인2(lb.F2정수64(g.Execprc))
...
func New현물_주문_정정(b []byte) (값 *S현물_주문_응답_실시간_정보, 에러 error) {
	...
	값.M응답_구분 = F2주문_응답_구분(g.Trcode)
	값.M종목코드 = 종목코드
	값.M수량 = lb.F확인2(lb.F2정수64(g.Mdfycnfqty))
	값.M가격 = lb.F확인2(lb.F2정수64(g.Mdfycnfprc))
	값.M잔량 = lb.F확인2(lb.F2정수64(g.Unercqty))
	값.M시각 = 시각
...
func New현물_주문_취소(b []byte) (값 *S현물_주문_응답_실시간_정보, 에러 error) {
	...
	값.M응답_구분 = F2주문_응답_구분(g.Trcode)
	값.M종목코드 = 종목코드
	값.M수량 = lb.F확인2(lb.F2정수64(g.Canccnfqty))
	값.M잔량 = lb.F확인2(lb.F2정수64(g.Orgordunercqty))
	값.M시각 = 시각
...
func New코스피_호가_잔량(b []byte) (값 *S호가_잔량_실시간_정보, 에러 error) {
	...
	값.M시각 = lb.F확인2(lb.F2금일_시각("150405", g.Hotime))
	값.M동시호가_구분 = T동시호가_구분(lb.F확인2(lb.F2정수64(g.Donsigubun)))
	값.M배분적용_구분 = lb.F2참거짓(g.Gubun, " ", false)
	...
	if len(매도호가_모음) != len(매도잔량_모음) {
		return nil, lb.New에러("매도호가, 매도잔량 수량이 서로 다름. %v %v",
			len(매도호가_모음), len(매도잔량_모음))
	}

	if len(매수호가_모음) != len(매수잔량_모음) {
		return nil, lb.New에러("매수호가, 매수잔량 수량이 서로 다름. %v %v",
	...
	값.M매도_총잔량 = lb.F확인2(lb.F2정수64(g.Totofferrem))
	값.M매수_총잔량 = lb.F확인2(lb.F2정수64(g.Totbidrem))

	값.M중간가격 = lb.F확인2(lb.F2정수64(g.Midprice))
	값.M매도중간가잔량합계수량 = lb.F확인2(lb.F2정수64(g.Offermidsumrem))
	값.M매수중간가잔량합계수량 = lb.F확인2(lb.F2정수64(g.Bidmidsumrem))
	값.M중간가잔량합계수량 = lb.F확인2(lb.F2정수64(g.Midsumrem))
...
func New코스피_시간외_호가_잔량(b []byte) (값 *S코스피_시간외_호가_잔량_실시간_정보, 에러 error) {
	...
	값.M종목코드 = lb.F2문자열(g.Shcode)
	값.M시각 = lb.F확인2(lb.F2금일_시각("150405", g.Hotime))
	값.M매도잔량 = lb.F확인2(lb.F2정수64(g.Tmofferrem))
	값.M매수잔량 = lb.F확인2(lb.F2정수64(g.Tmbidrem))
	값.M매도수량_직전대비 = lb.F확인2(lb.F2정수64(g.Pretmoffercha))
	값.M매수수량_직전대비 = lb.F확인2(lb.F2정수64(g.Pretmbidcha))
...
func New코스피_체결(b []byte) (값 *S코스피_체결, 에러 error) {
	...
	값 = new(S코스피_체결)
	값.M종목코드 = lb.F2문자열(g.Shcode)
	값.M시각 = lb.F확인2(lb.F2금일_시각("150405", g.Chetime))
	값.M전일대비구분 = T전일대비_구분(lb.F확인2(lb.F2정수64(g.Sign)))
	값.M전일대비등락폭 = lb.F확인2(lb.F2정수64(g.Change))
	값.M전일대비등락율 = lb.F확인2(lb.F2실수(g.Drate))
	값.M현재가 = lb.F확인2(lb.F2정수64(g.Price))
	값.M시가시각 = lb.F확인2(lb.F2금일_시각("150405", g.Opentime))
	값.M시가 = lb.F확인2(lb.F2정수64(g.Open))
	값.M고가시각 = lb.F확인2(lb.F2금일_시각("150405", g.Hightime))
	값.M고가 = lb.F확인2(lb.F2정수64(g.High))
	값.M저가시각 = lb.F확인2(lb.F2금일_시각("150405", g.Lowtime))
	값.M저가 = lb.F확인2(lb.F2정수64(g.Low))

	switch lb.F2문자열(g.Cgubun) {
	case "+":
		값.M매도_매수_구분 = lb.P매수
	case "-":
		값.M매도_매수_구분 = lb.P매도
	default:
		panic(lb.New에러("예상하지 못한 체결구분 값 : '%v'", lb.F2문자열(g.Cgubun)))
	}

	값.M체결량 = lb.F확인2(lb.F2정수64(g.Cvolume))
	값.M누적거래량 = lb.F확인2(lb.F2정수64(g.Volume))
	값.M누적거래대금 = lb.F확인2(lb.F2정수64(g.Value))
	값.M매도누적체결량 = lb.F확인2(lb.F2정수64(g.Mdvolume))
	값.M매도누적체결건수 = lb.F확인2(lb.F2정수64(g.Mdchecnt))
	값.M매수누적체결량 = lb.F확인2(lb.F2정수64(g.Msvolume))
	값.M매수누적체결건수 = lb.F확인2(lb.F2정수64(g.Mschecnt))
	값.M체결강도 = lb.F확인2(lb.F2실수(g.Cpower))
	값.M가중평균가 = lb.F확인2(lb.F2정수64(g.Avrg))
	값.M매도호가 = lb.F확인2(lb.F2정수64(g.Offerho))
	값.M매수호가 = lb.F확인2(lb.F2정수64(g.Bidho))

	switch lb.F2문자열_공백_제거(g.Status) {
	case "0", "00":
		값.M장_정보 = lb.P장_중
	case "4", "04":
		값.M장_정보 = lb.P장_후_시간외
	case "10":
		값.M장_정보 = lb.P장_전_시간외
	default:
		panic(lb.New에러("예상하지 못한 장 정보 값 : '%v'", lb.F2문자열_공백_제거(g.Status)))
	}

	값.M전일동시간대거래량 = lb.F확인2(lb.F2정수64(g.Jnilvolume))
...
func New코스피_예상_체결(b []byte) (값 *S코스피_예상_체결, 에러 error) {
	...
	값.M종목코드 = lb.F2문자열(g.Shcode)
	값.M시각 = lb.F확인2(lb.F2금일_시각("150405", g.Hotime))
	값.M예상체결가격 = lb.F확인2(lb.F2정수64(g.Yeprice))
	값.M예상체결수량 = lb.F확인2(lb.F2정수64(g.Yevolume))
	값.M예상체결가전일종가대비구분 = T전일대비_구분(lb.F확인2(lb.F2정수64(g.Jnilysign)))
	값.M예상체결가전일종가대비등락폭 = lb.F확인2(lb.F2정수64(g.Preychange))
	값.M예상체결가전일종가대비등락율 = lb.F확인2(lb.F2실수(g.Jnilydrate))
	값.M예상매도호가 = lb.F확인2(lb.F2정수64(g.Yofferho0))
	값.M예상매수호가 = lb.F확인2(lb.F2정수64(g.Ybidho0))
	값.M예상매도호가수량 = lb.F확인2(lb.F2정수64(g.Yofferrem0))
	값.M예상매수호가수량 = lb.F확인2(lb.F2정수64(g.Ybidrem0))
	값.M거래소_구분 = F2거래소_구분(g.Exchname)
...
func New코스피_ETF_NAV(b []byte) (값 *S코스피_ETF_NAV, 에러 error) {
	...
	값 = new(S코스피_ETF_NAV)
	값.M종목코드 = lb.F2문자열(g.Shcode)
	값.M시각 = lb.F확인2(lb.F2금일_시각("15:04:05", g.Time))
	값.M현재가 = lb.F확인2(lb.F2정수64(g.Price))
	값.M전일대비구분 = T전일대비_구분(lb.F확인2(lb.F2정수64(g.Sign)))
	값.M전일대비등락폭 = lb.F확인2(lb.F2정수64(g.Change))
	...
	값.M지수전일대비등락폭 = lb.F2실수_단순형_공백은_0(g.Jichange)
	값.M지수전일대비등락율 = lb.F2실수_단순형_공백은_0(g.Jirate)
...
func New주식_VI발동해제(b []byte) (값 *S주식_VI발동해제, 에러 error) {
	...
	값 = new(S주식_VI발동해제)
	값.M종목코드 = lb.F2문자열(g.Shcode)
	값.M참조코드 = lb.F2문자열(g.Ref_shcode)
	값.M시각 = lb.F확인2(lb.F2금일_시각("150405", g.Time))
	값.M구분 = VI발동해제(lb.F확인2(lb.F2정수64(g.Vi_gubun)))
	값.M정적VI발동_기준가격 = lb.F확인2(lb.F2정수64(g.Svi_recprice))
	값.M동적VI발동_기준가격 = lb.F확인2(lb.F2정수64(g.Dvi_recprice))
	값.VI발동가격 = lb.F확인2(lb.F2정수64(g.Vi_trgprice))
	값.M거래소_구분 = F2거래소_구분(g.Exchname)
...
func New장_운영정보(b []byte) (값 *S장_운영정보, 에러 error) {
	...
	값 = new(S장_운영정보)
	값.M장_구분 = T시장구분(lb.F2문자열(g.Jangubun))
	값.M장_상태 = T시장상태(lb.F확인2(lb.F2정수(g.Jstatus)))
