...
func New현물_주문_접수(b []byte) (값 *S현물_주문_응답_실시간_정보, 에러 error) {
	...
	종목코드 := lib.F2문자열_앞뒤_공백제거(g.Shtcode)
...
func New현물_주문_체결(b []byte) (값 *S현물_주문_응답_실시간_정보, 에러 error) {
	...
	종목코드 := lib.F2문자열_앞뒤_공백제거(g.ShtnIsuno)
	종목코드 = 종목코드[1:] // 맨 앞의 'A' 제거
...
func New코스피_체결(b []byte) (값 *S코스피_체결, 에러 error) {
	...
	switch lib.F2문자열_앞뒤_공백제거(g.Status) {
	case "0", "00":
		값.M장_정보 = lib.P장_중
	case "4", "04":
	...
	default:
		panic(lib.New에러("예상하지 못한 장 정보 값 : '%v'", lib.F2문자열_앞뒤_공백제거(g.Status)))
	}
