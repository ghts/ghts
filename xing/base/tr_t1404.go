...
func NewT1404InBlock(질의값 *T1404_관리종목_조회_질의값) (g *T1404InBlock) {
	...
	lb.F바이트_복사_문자열(g.Jongchk[:], strconv.Itoa(int(질의값.M관리_질의_구분)))
...
func NewT1404_관리종목_조회_응답_헤더(b []byte) (값 *T1404_관리종목_조회_응답_헤더, 에러 error) {
	...
	lb.F조건부_패닉(len(b) != SizeT1404OutBlock,
		"예상하지 못한 길이 : '%v", len(b))
...
func NewT1404_관리종목_조회_응답_반복값_모음(b []byte) (값_모음 *T1404_관리종목_조회_응답_반복값_모음, 에러 error) {
	...
	for i, g := range g_모음 {
		...
		값.M지정일_주가 = lb.F확인2(lb.F2정수64_공백은_0(g.Tprice))
		//값.M지정일_대비_등락폭 = lb.F확인2(lb.F2정수64(g.Tchange)
		//값.M지정일_대비_등락율 = lb.F확인2(lb.F2실수_소숫점_추가(g.Tdiff, 2)
		값.M사유 = T관리종목_지정_사유_구분(lb.F확인2(lb.F2정수64_공백은_0(g.Reason)))
