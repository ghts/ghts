...
func NewCSPAQ22200_현물계좌_예수금_주문가능금액_응답(b []byte) (값 *CSPAQ22200_현물계좌_예수금_주문가능금액_응답, 에러 error) {
	...
	lib.F확인(binary.Read(bytes.NewBuffer(b), binary.BigEndian, g_all)) // 네트워크 전송 바이트 순서는 빅엔디언.
	...
	값.M코스피금액 = lib.F2정수64_단순형(g2.SeOrdAbleAmt)
	값.M코스닥금액 = lib.F2정수64_단순형(g2.KdqOrdAbleAmt)
	...
	값.D1_예수금 = lib.F2정수64_단순형(g2.D1Dps)
	값.D2_예수금 = lib.F2정수64_단순형(g2.D2Dps)
