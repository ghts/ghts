...
func NewCFOAT00100InBlock1(질의값 *CFOAT00100_선물옵션_정상주문_질의값, 비밀번호 string) (g *CFOAT00100InBlock1) {
	...
	lib.F바이트_복사_정수(g.BnsTpCode[:], int(질의값.M매매구분))
...
func newCFOAT00100_선물옵션_정상주문_응답1(b []byte) (값 *CFOAT00100_선물옵션_정상주문_응답1, 에러 error) {
	...
	//값.M주문시장 = T주문시장구분(lib.F확인2(lib.F2정수(g.OrdMktCode))	// 해당 필드는 운영/모의투자 서버 동일하게 값이 40으로 고정되어 있습니다.
	값.M계좌번호 = lib.F2문자열_공백제거(g.AcntNo)
	값.M종목코드 = lib.F2문자열_공백제거(g.FnoIsuNo)
	값.M매매구분 = lib.T매도_매수_구분(lib.F확인2(lib.F2정수(g.BnsTpCode))
...
func newCFOAT00100_선물옵션_정상주문_응답2(b []byte) (값 *CFOAT00100_선물옵션_정상주문_응답2, 에러 error) {
	...
	lib.F조건부_패닉(len(b) != SizeCFOAT00100OutBlock2, "예상하지 못한 길이 : '%v", len(b))
