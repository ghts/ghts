...
func NewT3341_재무순위_응답_반복값_모음(b []byte) (값 *T3341_재무순위_응답_반복값_모음, 에러 error) {
	...
	for i, g := range g_모음 {
		...
		s.M매출액_증가율 = lib.F확인2(lib.F2실수_소숫점_추가_공백은_0(g.Salesgrowth, 2))
		s.M영업이익_증가율 = lib.F확인2(lib.F2실수_소숫점_추가_공백은_0(g.Operatingincomegrowt, 2))
		s.M경상이익_증가율 = lib.F확인2(lib.F2실수_소숫점_추가_공백은_0(g.Ordinaryincomegrowth, 2))
