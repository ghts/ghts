...
func NewT1310InBlock(질의값 *T1310_현물_전일당일분틱조회_질의값) (g *T1310InBlock) {
	g = new(T1310InBlock)
	lib.F바이트_복사_문자열(g.Daygb[:], strconv.Itoa(int(질의값.M당일_전일_구분)))
	lib.F바이트_복사_문자열(g.Timegb[:], strconv.Itoa(int(질의값.M분틱_구분)))
...
func NewT1310_현물_전일당일_분틱_조회_질의값_단순() *T1310_현물_전일당일분틱조회_질의값 {
	...
	s.S질의값_단일_종목.S질의값_기본형 = lib.New질의값_기본형(TR조회, TR현물_당일_전일_분틱_조회_t1310)
...
func NewT1310_현물_당일전일분틱조회_응답_반복값_모음(b []byte) (값 *T1310_현물_전일당일분틱조회_응답_반복값_모음, 에러 error) {
	...
	for i, g := range g_모음 {
		...
		s.M거래소_구분 = F2거래소_구분(lib.F2문자열_공백_제거(g.Exchname))
