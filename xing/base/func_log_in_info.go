...
func F로그인_정보_환경_변수_읽기() (로그인_정보 *S로그인_정보, 에러 error) {
	...
	} else if 계좌_비밀번호 := os.Getenv(P환경변수_계좌_비밀번호); F서버_구분() == P서버_실거래 && !lib.F테스트_모드_실행_중() && 계좌_비밀번호 == "" {
		return nil, lib.New에러("계좌 암호가 설정되어 있지 않습니다. 환경변수 '%v'에 설정하십시오.", P환경변수_계좌_비밀번호)
	} else {
		모의투자_암호 := os.Getenv(P환경변수_모의투자_암호)
		if 모의투자_암호 == "" {
			lib.F문자열_출력("모의투자 암호가 설정되어 있지 않습니다. 환경변수 '%v'에 설정하십시오.", P환경변수_로그인_암호)
		}
