...
func NewT1901_ETF_시세_조회_응답(b []byte) (s *T1901_ETF_시세_조회_응답, 에러 error) {
	...
	s.M업종_현재가 = lib.F2실수_소숫점_추가_단순형(g.Upprice, 2)
	s.M업종_전일대비구분 = T전일대비_구분(lib.F2정수64_단순형(g.Upsign))
	s.M업종_전일대비등락폭 = s.M업종_전일대비구분.G부호보정_실수64(lib.F2실수_소숫점_추가_단순형(g.Upchange, 2))
	s.M업종_전일대비등락율 = s.M업종_전일대비구분.G부호보정_실수64(lib.F2실수_소숫점_추가_단순형(g.Updiff, 2))
	...
	s.M선물_전일대비등락폭 = s.M선물_전일대비구분.G부호보정_실수64(lib.F2실수_소숫점_추가_단순형(g.Futchange, 2))
	s.M선물_전일대비등락율 = s.M선물_전일대비구분.G부호보정_실수64(lib.F2실수_소숫점_추가_단순형(g.Futdiff, 2))
	s.NAV = lib.F2실수_소숫점_추가_단순형(g.Nav, 2)
	s.NAV_전일대비구분 = T전일대비_구분(lib.F2정수64_단순형(g.Navsign))
	s.NAV_전일대비등락폭 = s.NAV_전일대비구분.G부호보정_실수64(lib.F2실수_소숫점_추가_단순형(g.Navchange, 2))
	s.NAV_전일대비등락율 = s.NAV_전일대비구분.G부호보정_실수64(lib.F2실수_소숫점_추가_단순형(g.Navdiff, 2))
	s.M추적_오차율 = lib.F2실수_소숫점_추가_단순형(g.Cocrate, 2)
	s.M괴리율 = lib.F2실수_소숫점_추가_단순형(g.Kasis, 2)
	...
	s.M참고지수현재가 = lib.F2실수_소숫점_추가_단순형(g.Upprice2, 2)

	s.M전일NAV = lib.F2실수_소숫점_추가_단순형(g.Jnilnav, 2)
	s.M전일NAV_전일대비구분 = T전일대비_구분(lib.F2정수64_단순형(g.Jnilnavsign))
	s.M전일NAV_전일대비등락폭 = lib.F2실수_소숫점_추가_단순형(g.Jnilnavchange, 2)
	s.M전일NAV_전일대비등락율 = lib.F2실수_소숫점_추가_단순형(g.Jnilnavdiff, 2)

	s.M순자산총액_억 = lib.F2정수64_단순형(g.Etftotcap)
	s.M스프레드 = lib.F2실수_소숫점_추가_단순형(g.Spread, 2)
	...
	if lib.F2문자열(g.Payday) == "00000000" {
		...
		s.ETN지급일 = lib.F2포맷된_일자_단순형("20060102", g.Payday)
	}
