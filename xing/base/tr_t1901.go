...
func NewT1901_ETF_시세_조회_응답(b []byte) (s *T1901_ETF_시세_조회_응답, 에러 error) {
	...
	s.M전일_거래량 = lib.F확인2(lib.F2실수(g.Jnilvolume))
	...
	s.NAV = lib.F확인2(lib.F2실수_소숫점_추가(g.Nav, 2))
	...
	s.M전일NAV_전일대비구분 = T전일대비_구분(lib.F확인2(lib.F2정수64_공백은_0(g.Jnilnavsign)))
