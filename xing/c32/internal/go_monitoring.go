...
func Go루틴_관리(ch초기화 chan lib.T신호) (에러 error) {
	...
	ch도우미_초기화 := make(chan lib.T신호, 2+수신_도우미_수량+콜백_도우미_수량)
	...
	go go실시간_정보_도우미(ch도우미_초기화, ch실시간_정보_도우미_종료)

	// Go루틴 초기화 대기
	for i := 0; i < (수신_도우미_수량 + 콜백_도우미_수량 + 2); i++ {
		<-ch도우미_초기화
	}
	...
	for {
		...
		case <-ch콜백_도우미_종료:
			go go콜백_도우미(ch도우미_초기화, ch콜백_도우미_종료)
		case <-ch실시간_정보_도우미_종료:
			go go실시간_정보_도우미(ch도우미_초기화, ch호출_도우미_종료)
		}
	}
}
