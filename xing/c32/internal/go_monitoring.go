...
func Go루틴_관리(ch초기화_완료 chan lib.T신호) (에러 error) {
	lib.S예외처리{M에러: &에러, M함수_항상: func() {
		Ch모니터링_루틴_종료 <- lib.P신호_종료
	}}.S실행()

	ch초기화 := make(chan lib.T신호, 2+수신_도우미_수량+콜백_도우미_수량)
	...
	// Go루틴 생성
	go go_HTTP서버(ch초기화, ch_HTTP서버_종료)
	<-ch초기화

	go go함수_호출_도우미(ch초기화, ch호출_도우미_종료)
	<-ch초기화

	for i := 0; i < 수신_도우미_수량; i++ {
		go go수신_도우미(ch초기화, ch수신_도우미_종료)
	}

	for i := 0; i < 콜백_도우미_수량; i++ {
		go go콜백_도우미(ch초기화, ch콜백_도우미_종료)
	}
