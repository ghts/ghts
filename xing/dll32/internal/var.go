...
// 다중 사용에 안전한 값들.
var (
	소켓REP_TR수신   = lb.F확인2(nano.NewNano소켓REP(xt.F주소_DLL32()))
	소켓PUB_실시간_정보 = lb.F확인2(nano.NewNano소켓PUB(xt.F주소_실시간()))

	소켓REQ_저장소 = lb.New소켓_저장소(20, func() (lb.I소켓_질의, error) {
		return nano.NewNano소켓REQ(xt.F주소_콜백(), lb.P30초)
	})
	...
	Ch질의  = make(chan *lb.S채널_질의, 100)
	ch콜백  = make(chan lb.I콜백, 100)

	수신_도우미_수량 = lb.F최대값(runtime.NumCPU(), 2)
	콜백_도우미_수량 = lb.F최대값(runtime.NumCPU(), 2)

	Ch모니터링_루틴_종료   = make(chan lb.T신호, 1)
	Ch함수_호출_도우미_종료 = make(chan lb.T신호, 1)
	Ch수신_도우미_종료    = make(chan lb.T신호, 수신_도우미_수량)
	Ch콜백_도우미_종료    = make(chan lb.T신호, 콜백_도우미_수량)

	API_초기화_완료 = lb.New안전한_bool(false)
