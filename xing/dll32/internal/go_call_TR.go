...
func f질의값_처리(질의 *lb.S채널_질의) {
	...
	//	F메시지_윈도우_생성()
	default:
		panic(lb.New에러("예상하지 못한 TR구분값 : '%v'", int(질의.M값.TR구분())))
	}
}
...
func F조회_및_주문_질의_처리(질의 *lb.S채널_질의) {
	...
	case xt.TR현물_기간별_조회_t1305:
		연속키 := lb.F2문자열_공백_제거(질의값.(*xt.T1305_현물_기간별_조회_질의값).M연속키)
		if 연속키 != "" {
			연속_조회_여부 = true
			연속_조회_키 = 연속키
		}
	...
	case xt.TR현물_당일_전일_분틱_조회_t1310:
		연속키 := lb.F2문자열_공백_제거(질의값.(*xt.T1310_현물_전일당일분틱조회_질의값).M연속키)
		if 연속키 != "" {
			연속_조회_여부 = true
			연속_조회_키 = 연속키
		}
	...
	case xt.TR관리_불성실_투자유의_조회_t1404:
		연속키 := lb.F2문자열_공백_제거(질의값.(*xt.T1404_관리종목_조회_질의값).M연속키)
		if 연속키 != "" {
			연속_조회_여부 = true
			연속_조회_키 = 연속키
		}
	...
	case xt.TR투자경고_매매정지_정리매매_조회_t1405:
		연속키 := lb.F2문자열_공백_제거(질의값.(*xt.T1405_투자경고_조회_질의값).M연속키)
		if 연속키 != "" {
			연속_조회_여부 = true
			연속_조회_키 = 연속키
		}
	...
	case xt.TR_ETF_시세_조회_t1901:
		c데이터 = unsafe.Pointer(xt.NewT1901InBlock(질의값.(*lb.S질의값_단일_종목)))
		길이 = xt.SizeT1901InBlock
	case xt.TR_ETF_시간별_추이_t1902:
		연속키 := lb.F2문자열_공백_제거(질의값.(*lb.S질의값_단일종목_연속키).M연속키)
		if 연속키 != "" {
			연속_조회_여부 = true
			연속_조회_키 = 연속키
		}
		c데이터 = unsafe.Pointer(xt.NewT1902InBlock(질의값.(*lb.S질의값_단일종목_연속키)))
		길이 = xt.SizeT1902InBlock
	case xt.TR_ETF_LP호가_조회_t1906:
		c데이터 = unsafe.Pointer(xt.NewT1906InBlock(질의값.(*lb.S질의값_단일_종목)))
		길이 = xt.SizeT1906InBlock
	case xt.TR기업정보_요약_t3320:
		c데이터 = unsafe.Pointer(xt.NewT3320InBlock(질의값.(*lb.S질의값_단일_종목)))
	...
	case xt.TR현물_멀티_현재가_조회_t8407:
		c데이터 = unsafe.Pointer(xt.NewT8407InBlock(질의값.(*lb.S질의값_복수_종목)))
		길이 = xt.SizeT8407InBlock
	case xt.TR현물_차트_일주월년_t8410:
		연속키 := lb.F2문자열_공백_제거(질의값.(*xt.T8410_현물_차트_일주월년_질의값).M연속일자)
	...
	case xt.TR현물_차트_틱_t8411:
		연속키 := lb.F2문자열_공백_제거(질의값.(*xt.T8411_현물_차트_틱_질의값).M연속일자) +
			lb.F2문자열_공백_제거(질의값.(*xt.T8411_현물_차트_틱_질의값).M연속시간)
	...
	case xt.TR현물_차트_분_t8412:
		연속키 := lb.F2문자열_공백_제거(질의값.(*xt.T8412_현물_차트_분_질의값).M연속일자) +
			lb.F2문자열_공백_제거(질의값.(*xt.T8412_현물_차트_분_질의값).M연속시간)
	...
	default:
		panic(lb.New에러("미구현 : '%v'", TR코드))
	}

	lb.F조건부_패닉(c데이터 == nil, "c데이터 설정 실패.")

	식별번호 := lb.F확인2(F질의(TR코드, c데이터, 길이, 연속_조회_여부, 연속_조회_키, lb.P30초))

	switch {
	case 식별번호 < 0:
		질의.Ch에러 <- lb.New에러("TR호출 실패. 반환된 식별번호가 음수임. '%v' '%v'", TR코드, 식별번호)
	default:
		질의.Ch회신값 <- 식별번호
	}
}

func F실시간_정보_구독_해지_처리(질의 *lb.S채널_질의) {
	...
	case xt.TR실시간_정보_해지:
		함수 = F실시간_정보_해지
	default:
		panic(lb.F2문자열("예상하지 못한 경우 : '%v' '%v'", int(TR구분), TR구분.String()))
	}
...
func F종료_질의_처리(질의 *lb.S채널_질의) {
	질의.Ch회신값 <- lb.P신호_종료

	lb.F대기(lb.P500밀리초) // 회신 전달 대기

	f종료()
}

func f종료() {
	f콜백_동기식(lb.New콜백_신호(lb.P신호_DLL32_종료))
	f실시간_정보_일괄_해지()
	F로그아웃()
	F소켓_정리()
	OnDestroy()
