...
func TestI질의_I회신(테스트 *testing.T) {
	...
	회신 = New질의(P메시지_SET, "키1", "값1").G회신(ch질의)
	F테스트_에러없음(테스트, 회신.G에러())
	F테스트_같음(테스트, 회신.G길이(), 0)

	회신 = New질의(P메시지_SET, "키2", "값2-1", "값2-2").G회신(ch질의)
	F테스트_에러없음(테스트, 회신.G에러())
	F테스트_같음(테스트, 회신.G길이(), 0)

	회신 = New질의(P메시지_SET, "키3", "값3-1", "값3-2", "값3-3").G회신(ch질의)
	F테스트_에러없음(테스트, 회신.G에러())
	F테스트_같음(테스트, 회신.G길이(), 0)

	회신 = New질의(P메시지_GET, "키1").G회신(ch질의)
	F테스트_에러없음(테스트, 회신.G에러())
	F테스트_같음(테스트, 회신.G길이(), 2)
...
func TestI종목별_보유량(테스트 *testing.T) {
	...
	정수값 = int(종목별_보유량.G롱포지션() * -1 - 10)
	F테스트_에러발생(테스트, 종목별_보유량.S더하기_롱포지션(정수값))
	
	정수값 = int(종목별_보유량.G숏포지션() * -1 - 10)
