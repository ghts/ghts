...
func TestI질의(테스트 *testing.T) {
	테스트.Parallel()
	
	회신_채널 := make(chan I메시지, 1)
	
	질의 := New질의(회신_채널, P메시지_GET, "질의", 1)
	F테스트_같음(테스트, 질의.G구분(), P메시지_GET)
	F테스트_같음(테스트, 질의.G내용()[0], "질의")
	F테스트_같음(테스트, 질의.G내용()[1], "1")
	
	질의.G회신_채널() <- New메시지(P메시지_OK, "회신", 2)
	회신_메시지 := <-회신_채널 
	F테스트_같음(테스트, 회신_메시지.G구분(), P메시지_OK)
	F테스트_같음(테스트, 회신_메시지.G내용()[0], "회신")
	F테스트_같음(테스트, 회신_메시지.G내용()[1], "2")
}
