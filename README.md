...
증권사 호출 기능은 2개로 분리됨.
- 동시 처리는 항상 데이터 동시 액세스 충돌 (Data Race : 데이터 레이스) 에러 발생 위험이 상존함.    
- Go언어에서는 데이터 레이스 감지기(Go Data Race Detector)가 64비트에서만 지원됨. (https://golang.org/doc/articles/race_detector.html)
- 복수의 매매 전략을 동시에 실행할 때, 데이터 레이스 버그 발생 우려가 있으므로, 매매 전략은 64비트에서 실행해야 함.
- 국내 모든 증권사 API는 32비트로만 호출할 수 있음.
- 매매 전략 실행은 64비트에서 하되, 증권사 API는 32비트에서 호출하기 위해서 독립된 2개의 프로세스로 분리.
- 더 자세한 사항은 https://ghts.tistory.com/48 참조
*********************************************************
