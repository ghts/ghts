...
func f_ETF_현재가_조회_ETF_자료_테스트(테스트 *testing.T,
	...
	오차율 := 공용.F오차율(ETF_자료.M괴리율, 괴리율_근사값)
	오차 := math.Abs(ETF_자료.M괴리율 - 괴리율_근사값)
	공용.F테스트_참임(테스트, 오차율 < 1 || 오차 < 0.02, ETF_자료.M괴리율, 괴리율_근사값)
	...
	공용.F테스트_참임(테스트, 
		ETF_자료.ETF_복제_방법_구분_코드 == P실물복제 ||
		ETF_자료.ETF_복제_방법_구분_코드 == P합성복제, 
		ETF_자료.ETF_복제_방법_구분_코드)
	
	공용.F테스트_참임(테스트, 
		ETF_자료.ETF_상품_유형_코드 == P일반형 ||
		ETF_자료.ETF_상품_유형_코드 == P파생형, 
		ETF_자료.ETF_상품_유형_코드)
}
