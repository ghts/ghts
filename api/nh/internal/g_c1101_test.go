...
func f주식_현재가_조회_기본_자료_테스트(테스트 *testing.T,
	...
	금일_9시 := time.Date(지금.Year(), 지금.Month(), 지금.Day(), 9, 0, 0, 0, 지금.Location())
	...
	거래량_비율_근사값 := float64(기본_자료.M거래량) / float64(기본_자료.M전일_거래량) * 100
	공용.F테스트_참임(테스트, 공용.F오차율(기본_자료.M전일대비_거래량_비율, 거래량_비율_근사값) < 10,
		기본_자료.M전일대비_거래량_비율, 거래량_비율_근사값, 기본_자료.M거래량, 기본_자료.M전일_거래량)
	...
	case 지금.Before(금일_9시) || 지금.After(금일_15시): // 장외
		공용.F테스트_참임(테스트, 기본_자료.M거래원_정보_수신_시각.Hour() == 15, 기본_자료.M거래원_정보_수신_시각)
	default:
		공용.F에러("예상하지 못한 경우")
		테스트.FailNow()
	}
	...
	case 0:
		공용.F테스트_거짓임(테스트, 기본_자료.M대량_매매_존재, 기본_자료.M대량_매매_존재)
	case 1, 2:
		공용.F테스트_참임(테스트, 기본_자료.M대량_매매_존재, 기본_자료.M대량_매매_존재)
	default:
...
func f주식_현재가_조회_변동_거래량_자료_테스트(테스트 *testing.T,
	...
	공용.F메모("금일이 개장일인지 여부 및 가장 최근 개장일 데이터가 필요함.\n" +
		"신용잔고 기준 결제일 다음날을 사용하면 되지 않을까?\n" +
		"변동 거래량 생성할 때 이것을 알 수 있도록 전달할 수 있을까?\n")
	...
	금일_14시 := time.Date(지금.Year(), 지금.Month(), 지금.Day(), 14, 0, 0, 0, 지금.Location())
	금일_15시 := 금일_14시.Add(time.Hour)
	금일_16시 := 금일_15시.Add(time.Hour)
	...
	// 월요일에는 금요일이 '신용잔고_기준_결제일'이 됨.
	//공용.F문자열_출력("결제일 : %v", 기본_자료.M신용잔고_기준_결제일)

	for i, 변동_자료 := range 변동_자료_모음 {	
		switch {
		case 변동_자료.M시각.After(삼분후):
			공용.F에러("잘못된 시각. %v", 변동_자료.M시각)
			테스트.FailNow()
		case 변동_자료.M시각.Before(금일_0시): // 과거 자료.
			공용.F테스트_참임(테스트, 변동_자료.M시각.IsZero(), 변동_자료.M시각)
		case 지금.After(개장_시각) && 지금.Before(폐장_시각): // 장중
			// 컴퓨터 시계에 오차가 존재하니 약간 여유를 둬야 함.
			공용.F테스트_참임(테스트, 변동_자료.M시각.After(개장_시각) && 변동_자료.M시각.Before(삼분후), 변동_자료.M시각, 삼분전, 삼분후)
		default: // 장 마감
			공용.F테스트_참임(테스트, 변동_자료.M시각.After(금일_14시) && 변동_자료.M시각.Before(금일_16시), 변동_자료.M시각)
		}
		...
		if 지금.After(개장_시각) && 지금.Before(거래_마감_시각) { // 장중
			...
			if 변동_자료.M매도_호가 > 0 {
				공용.F테스트_참임(테스트, 변동_자료.M매도_호가 >= 기본_자료.M하한가 &&
					변동_자료.M매도_호가 <= 기본_자료.M상한가, 변동_자료.M매도_호가)
			}

			if 변동_자료.M매수_호가 > 0 {
				공용.F테스트_참임(테스트, 변동_자료.M매수_호가 >= 기본_자료.M하한가 &&
					변동_자료.M매수_호가 <= 기본_자료.M상한가, 변동_자료.M매수_호가)
			}
		}
		
		// 걸러낸 자료로 인한 오차 수정.
		if i == 0 && 변동_자료.M거래량 != 거래량_잔량 {
			거래량_잔량 = 변동_자료.M거래량
		}
