...
func TestCh조회_주식_현재가(테스트 *testing.T) {
	공용.F메모("stack split 에러가 발생하는 데 이유를 모르겠음.\n" +
			"콜백된 Go함수가 go루틴과 다른 스레드에서 동시에 실행되다가 에러가 나는걸까?\n" +
			"콜백된 Go함수가 go루틴의 채널로 전송해서 실제 처리는 하나의 스레드에서 실행되도록 하자.")
	...
	for !질의.G회신_종료() {
		...
		case P회신_조회:
			...
			case "c1101OutBlock3":
				...
				종목_지표_자료, ok = 수신_데이터.G데이터().(*S주식_현재가_조회_종목_지표)
				공용.F테스트_참임(테스트, ok)
			default:
	...
	for _, 변동_자료 := range 변동_자료_모음 {
		...
		} else if 변동_자료.M매도_호가 > 0 { // 장 마감 후 매도 호가가 0일 때가 있음.
			공용.F테스트_참임(테스트, 변동_자료.M매도_호가 >= 변동_자료.M현재가, 변동_자료.M매도_호가, 변동_자료.M현재가)
		}
	...
	// 종목 지표 테스트
	공용.F테스트_참임(테스트, 종목_지표_자료 != nil)
	공용.F테스트_참임(테스트, 종목_지표_자료.M동시_호가_구분 >= 0 && 종목_지표_자료.M동시_호가_구분 <= 6)
	
	
	if 지금.After(개장_시각) && 지금.Before(폐장_시각) { // 장중
		공용.F테스트_참임(테스트, 종목_지표_자료.M예상_체결가 <= 기본_자료.M상한가)
		공용.F테스트_참임(테스트, 종목_지표_자료.M예상_체결가 >= 기본_자료.M하한가, 종목_지표_자료.M동시_호가_구분, 종목_지표_자료.M예상_체결가, 기본_자료.M하한가)
		공용.F테스트_참임(테스트, 공용.F오차율(종목_지표_자료.M예상_체결가, 기본_자료.M현재가) < 10)
	} else if 종목_지표_자료.M예상_체결가 != 0 {	// 장외
		공용.F테스트_참임(테스트, 종목_지표_자료.M예상_체결가 <= 기본_자료.M상한가)
		공용.F테스트_참임(테스트, 종목_지표_자료.M예상_체결가 >= 기본_자료.M하한가, 종목_지표_자료.M동시_호가_구분, 종목_지표_자료.M예상_체결가, 기본_자료.M하한가)
		공용.F테스트_참임(테스트, 공용.F오차율(종목_지표_자료.M예상_체결가, 기본_자료.M현재가) < 10)
	}
	
	공용.F테스트_참임(테스트, f올바른_등락부호(종목_지표_자료.M예상_체결부호), 종목_지표_자료.M예상_체결부호)
	
	if 지금.After(개장_시각) && 지금.Before(폐장_시각) { // 장중
		공용.F테스트_같음(테스트, f등락부호2정수(종목_지표_자료.M예상_체결부호) * 종목_지표_자료.M예상_등락폭,
			 종목_지표_자료.M예상_체결가 - 기본_자료.M전일종가)
	}
	
	if 종목_지표_자료.M예상_등락폭 != 0 && 종목_지표_자료.M예상_등락율 != 0 {
		예상_등락율_근사값 := math.Abs(float64(종목_지표_자료.M예상_등락폭)) / 
			float64(종목_지표_자료.M예상_체결가) * 100
		공용.F테스트_참임(테스트, 공용.F오차율(종목_지표_자료.M예상_등락율, 예상_등락율_근사값) < 10)	
	}
	
	공용.F테스트_참임(테스트, 종목_지표_자료.M예상_체결수량 >= 0)
	공용.F테스트_참임(테스트, 종목_지표_자료.M예상_체결수량 <= 기본_자료.M매도_잔량_총합 ||
		종목_지표_자료.M예상_체결수량 <= 기본_자료.M매수_잔량_총합)
}
...
func f주식_현재가_조회_기본_자료_테스트(테스트 *testing.T, 기본_자료 *S주식_현재가_조회_기본_자료, 종목 공용.I종목) {
	...
	if 기본_자료.M매도_잔량_최우선 > 0 {
		...
		공용.F메모("매도 호가가 현재가 보다 낮은 경우가 발생함. 장외시간에만 그런 것인지, 버그인지 확인할 것.")
		공용.F테스트_참임(테스트, 기본_자료.M매도_호가_최우선 >= 기본_자료.M현재가)
	}
	...
	if 기본_자료.M매수_잔량_최우선 > 0 {
		...
		공용.F테스트_참임(테스트, 기본_자료.M매수_호가_최우선 <= 기본_자료.M현재가)
	}
