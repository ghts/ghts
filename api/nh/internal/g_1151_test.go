...
func TestCh조회_ETF_현재가(테스트 *testing.T) {
	...
	변동_자료_모음 := make([]S_ETF_현재가_조회_변동_거래량_자료, 0)
	...
	지수_자료 := new(S_ETF_현재가_조회_기반_지수_자료)
	지수_자료 = nil
	...
	for !완료_메시지_수신 || 기본_자료 == nil {
		...
		case P회신_조회:	
			...
			case "c1151OutBlock5":
				...
				지수_자료, ok = 수신_데이터.G데이터().(*S_ETF_현재가_조회_기반_지수_자료)
				공용.F테스트_참임(테스트, ok)
			default:
	...
	// 기반 지수 자료 테스트
	f_ETF_현재가_조회_기반_지수_자료_테스트(테스트, 기본_자료, 기반_지수_자료)
}
...
func f_ETF_현재가_조회_기본_자료_테스트(테스트 *testing.T) {
	...
	if 기본_자료.M매도_잔량_최우선 > 0 {
		...
		if 지금.After(개장_시각) && 지금.Before(거래_마감_시각) {
			공용.F테스트_참임(테스트, 기본_자료.M매도_호가_최우선 >= 기본_자료.M현재가 * 0.9, 기본_자료.M매도_호가_최우선, 기본_자료.M현재가 * 0.9)
		}
	}
	...
	if 기본_자료.M매수_잔량_최우선 > 0 {
		...
		// 시장가 주문이 체결 중에는 최우선 호가를 넘어서서 계속 진행 중일 수 있다.
		if 지금.After(개장_시각) && 지금.Before(거래_마감_시각) {
			공용.F테스트_참임(테스트, 기본_자료.M매수_호가_최우선 <= 기본_자료.M현재가 * 1.1, 기본_자료.M매수_호가_최우선, 기본_자료.M현재가 * 1.1)
		}
	}
	...
	공용.F테스트_참임(테스트, 공용.F오차율(기본_자료.M상한가, float64(기본_자료.M전일종가)*1.3) < 5)
	공용.F테스트_참임(테스트, 공용.F오차율(기본_자료.M하한가, float64(기본_자료.M전일종가)*0.7) < 5)
	...
	switch {
	case 기본_자료.M거래원_정보_수신_시간.After(삼분후):	// 미래 자료
		공용.F에러("잘못된 시각. %v", 기본_자료.M거래원_정보_수신_시간)
		테스트.FailNow()
	case 기본_자료.M거래원_정보_수신_시간.Before(금일_0시):	// 과거 자료.
		공용.F테스트_참임(테스트, 기본_자료.M거래원_정보_수신_시간.Hour() == 15, 기본_자료.M거래원_정보_수신_시간)	// 과거 장마감 시각
	case 지금.After(개장_시각) && 지금.Before(거래_마감_시각):	// 장중
		공용.F테스트_참임(테스트, 기본_자료.M거래원_정보_수신_시간.After(개장_시각) && 기본_자료.M시각.Before(삼분후), 기본_자료.M거래원_정보_수신_시간)
	default: // 장 마감 후
		공용.F테스트_참임(테스트, 기본_자료.M거래원_정보_수신_시간.After(금일_15시) && 기본_자료.M거래원_정보_수신_시간.Before(금일_16시), 기본_자료.M거래원_정보_수신_시간)
	}
...
func f_ETF_현재가_조회_변동_거래량_자료_테스트(테스트 *testing.T, 
	기본_자료 *S_ETF_현재가_조회_기본_자료, 
	변동_자료_모음 []S_ETF_현재가_조회_변동_거래량_자료) {
	...
	for _, 변동_자료 := range 변동_자료_모음 {
		...
		// 시장가 주문은 체결 중에는 호가가 변하지 않는다.
			// 시장가 주문이 체결 중에는 최우선 호가를 넘어서서 계속 진행 중일 수 있다.
			공용.F테스트_참임(테스트, 변동_자료.M매도_호가 >= 변동_자료.M현재가 * 0.9, 변동_자료.M매도_호가, 변동_자료.M현재가 * 0.9)
			공용.F테스트_참임(테스트, 변동_자료.M매수_호가 <= 변동_자료.M현재가 * 1.1, 변동_자료.M매수_호가, 변동_자료.M현재가 * 1.1)
		} else {
...
func f_ETF_현재가_조회_ETF_자료_테스트(테스트 *testing.T, 
	...
	공용.F테스트_참임(테스트, ETF_자료.NAV > 0, NAV)
	공용.F테스트_참임(테스트, f올바른_등락부호(ETF_자료.NAV_등락부호)
		
	공용.F문자열_출력("ETF NAV 등락폭 %v", ETF_자료.NAV_등락폭)
	NAV_근사값 := ETF_자료.M전일NAV + f등락부호2정수(ETF_자료.M등락부호) * ETF_자료.NAV_등락폭
	공용.F테스트_참임(테스트, 공용.F2오차율(ETF_자료.NAV, NAV_근사값) < 3) 

	공용.F문자열_출력("ETF 괴리율 %v", ETF_자료.M괴리율)
	
	공용.F테스트_참임(테스트, math.Abs(float64(ETF_자료.M괴리율)) < 10, ETF_자료.M괴리율)
	
	괴리율_근사값_1 := math.Abs(float64(기본_자료.M현재가 - ETF_자료.NAV)) / float64(기본_자료.M현재가)
	괴리율_근사값_2 := math.Abs(float64(기본_자료.M현재가 - ETF_자료.NAV)) / float64(ETF_자료.NAV)
	공용.F테스트_참임(테스트, 
		공용.F2오차율(ETF_자료.M괴리율, 괴리율_근사값_1) < 1 ||
		공용.F2오차율(ETF_자료.M괴리율, 괴리율_근사값_2) < 1, 
		ETF_자료.M괴리율, 괴리율_근사값_1, 괴리율_근사값_2)
	
	공용.F테스트_참임(테스트, f올바른_등락부호(ETF_자료.괴리율_부호))
	f테스트_등락율(테스트, 기본_자료.M괴리율_부호, 기본_자료.M괴리율)
	
	공용.F테스트_참임(테스트, ETF_자료.M설정단위_당_현금_배당액 >= 0)
	공용.F테스트_참임(테스트, ETF_자료.M구성_종목수 > 0)
	공용.F테스트_참임(테스트, ETF_자료.M순자산_총액_억 > 0)
	
	순자산_총액_근사값_1 := 기본_자료.M시가_총액_억 * (1 + f등락부호2정수(ETF_자료.M괴리율_부호) * ETF_자료.M괴리율)
	순자산_총액_근사값_2 := 기본_자료.M시가_총액_억 * (1 - f등락부호2정수(ETF_자료.M괴리율_부호) * ETF_자료.M괴리율)
	...
	if LP_매도_잔량_차선 > 0 {
		공용.F테스트_참임(테스트, LP_매도_잔량_최우선 > 0) 
	}
	
	if LP_매도_잔량_차차선 > 0 {
		공용.F테스트_참임(테스트, LP_매도_잔량_차선 > 0)
	}
	
	if LP_매도_잔량_4차선 > 0 {
		공용.F테스트_참임(테스트, LP_매도_잔량_차차선 > 0)
	}
	
	if LP_매도_잔량_5차선 > 0 {
		공용.F테스트_참임(테스트, LP_매도_잔량_4차선 > 0)
	}
	
	if LP_매도_잔량_6차선 > 0 {
		공용.F테스트_참임(테스트, LP_매도_잔량_5차선 > 0)
	}
	
	if LP_매도_잔량_7차선 > 0 {
		공용.F테스트_참임(테스트, LP_매도_잔량_6차선 > 0)
	}
	
	if LP_매도_잔량_8차선 > 0 {
		공용.F테스트_참임(테스트, LP_매도_잔량_7차선 > 0)
	}
	
	if LP_매도_잔량_9차선 > 0 {
		공용.F테스트_참임(테스트, LP_매도_잔량_8차선 > 0)
	}
	
	if LP_매도_잔량_10차선 > 0 {
		공용.F테스트_참임(테스트, LP_매도_잔량_9차선 > 0)
	}
	
	if LP_매수_잔량_차선 > 0 {
		공용.F테스트_참임(테스트, LP_매수_잔량_최우선 > 0) 
	}
	
	if LP_매수_잔량_차차선 > 0 {
		공용.F테스트_참임(테스트, LP_매수_잔량_차선 > 0)
	}
	
	if LP_매수_잔량_4차선 > 0 {
		공용.F테스트_참임(테스트, LP_매수_잔량_차차선 > 0)
	}
	
	if LP_매수_잔량_5차선 > 0 {
		공용.F테스트_참임(테스트, LP_매수_잔량_4차선 > 0)
	}
	
	if LP_매수_잔량_6차선 > 0 {
		공용.F테스트_참임(테스트, LP_매수_잔량_5차선 > 0)
	}
	
	if LP_매수_잔량_7차선 > 0 {
		공용.F테스트_참임(테스트, LP_매수_잔량_6차선 > 0)
	}
	
	if LP_매수_잔량_8차선 > 0 {
		공용.F테스트_참임(테스트, LP_매수_잔량_7차선 > 0)
	}
	
	if LP_매수_잔량_9차선 > 0 {
		공용.F테스트_참임(테스트, LP_매수_잔량_8차선 > 0)
	}
	
	if LP_매수_잔량_10차선 > 0 {
		공용.F테스트_참임(테스트, LP_매수_잔량_9차선 > 0)
	}
	
	공용.F문자열_출력("ETF 복제 방법 구분 코드 %v", ETF_복제_방법_구분_코드)
	공용.F문자열_출력("ETF_상품_유형_코드 %v", ETF_상품_유형_코드)		
}