...
func F_NH_OpenAPI_Go루틴(ch초기화 chan bool) {
	공용.F메모("C구조체 입력값 생성할 때 남는 공간을 공백문자로 채워넣을 것.\n" +
		"C구조체 입력값 생성할 때 생기는 모든 CString은 free()로 해제할 것.")

	//	공용.F메모("가끔씩 stack split 에러가 발생하는 데 이유를 모르겠음.\n" +
	//			"어쩌면 C문자열 변환 시 null문자 이후를 건드려서 그랬을 수도 있음. (지금은 수정함.)\n" +\
	//			"go루틴이 실행되는 스레드와 콜백된 Go함수가 실행되는 스레드가 달라서 발생한다면\n" +
	//			"콜백 Go함수가 직접 메시지를 처리하지 말고, 채널을 통해서 go루틴으로 전달한 후,\n"+
	//			"go루틴에서 처리함으로써 동일한 스레드에서 처리되도록 보장하면 됨.\n\n" +
...
func f조회_질의_처리(질의 공용.I질의_가변형) {
	...
	switch TR코드 {
	case TR주식_현재가_조회:
		if 질의.G검사(공용.P메시지_GET, 2) != nil {
			return
		}

		종목_코드 := 질의.G내용(1).(string)
		c데이터 = NewC1101InBlock(종목_코드)
		길이 = C.int(unsafe.Sizeof(C.Tc1101InBlock{}))
	...
	case TR_ETF_현재가_조회:
		...
		길이 = C.int(unsafe.Sizeof(C.Tc1151InBlock{}))
		계좌_인덱스 = 0
	case TR주식_매도:
		fallthrough
	case TR주식_매수:
	...
	case TR선물_현재가_조회,
		...
		TR주식선물_스프레드_현재가_조회,
		TR선물_옵션_코드_조회:
		에러 := 공용.F에러(" %v : 아직 미구현 TR코드. 향후 지원 예정.", TR코드)
		panic(에러)
	case TR_ELW_현재가_조회,
		TR_ELW_매도,
		TR_ELW_매수,
		...
		에러 := 공용.F에러("%v : 지원하지 않는 TR코드", TR코드)
		panic(에러)
	default:
		에러 := 공용.F에러("%v : 존재하지 않는 TR코드", TR코드)
		panic(에러)
	}

	if 길이 == 0 {
		에러 := 공용.F에러("입력 데이터 길이가 0임.")
		panic(에러)
	}
...
func f실시간_서비스_등록_질의_처리(질의 공용.I질의_가변형) {
	...
	default:
		...
		if !ok {
			에러 := 공용.F에러("G내용(4) 잘못된 자료형. chan I수신_데이터_NH 형이 아님.")
			질의.S회신(에러, 공용.P메시지_에러)
			panic(에러)
		}
	}
	...
	if !ok {
		에러 := 공용.F에러("실시간 서비스 등록 실패. %v %v %v %v",
...
func f실시간_서비스_해제_질의_처리(질의 공용.I질의_가변형) {
	...
	if !ok {
		에러 := 공용.F에러("실시간 서비스 해제 실패. %v %v %v %v",
			타입, 코드_모음, 단위_길이, 전체_길이)
		질의.S회신(에러, 공용.P메시지_에러)
		return
	}

	질의.S회신(nil, P회신_실시간_서비스_모두_해지)
}
