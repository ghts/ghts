	...
	for !질의.G회신_종료() {
		...
		case P회신_조회:
			...
			case "c1101OutBlock":
				...
				공용.F테스트_참임(테스트, s.M등락율 >= 0)
				
				if s.M현재가 != 0 && s.M등락폭 != 0 && s.M등락율 != 0 {
					// 퍼센트가 아니네.. 다시 100을 곱해야 함.
					등락율_근사값 := math.Abs(float64(s.M등락폭)) / float64(s.M현재가) * 100
					
					공용.F테스트_참임(테스트, 공용.F오차율(등락율_근사값, s.M등락율) < 10, 
						s.M등락율, 등락율_근사값, s.M등락폭, s.M현재가, 공용.F오차율(등락율_근사값, s.M등락율))
				}
				
				공용.F테스트_참임(테스트, s.M거래량 >= 0)
				
				공용.F메모("거래비율 정의가 뭐지?")
				//if s.M상장_주식_수량_1000주_단위 != 0 {
				//	거래비율_근사값 := float64(s.M거래량) / 
				//					float64(s.M상장_주식_수량_1000주_단위) * 100
				//	공용.F테스트_참임(테스트, 공용.F오차율(s.M거래비율, 거래비율_근사값) < 10, s.M거래비율, 거래비율_근사값)
				//}
					
				if s.M유동_주식수 != 0 {
					// 유동 주식수와 거래량의 단위가 다른 듯 하다.
					공용.F메모("거래량과 유동 주식수는 단위가 다름")
					유동주_회전율_근사값 := float64(s.M거래량) / float64(s.M유동_주식수) / 10
					공용.F테스트_참임(테스트, 공용.F오차율(s.M유동주_회전율, 유동주_회전율_근사값) < 10, s.M유동주_회전율, 유동주_회전율_근사값)
				}
				...
				if s.M거래대금 != 0 && s.M거래량 != 0 && s.M현재가 != 0 {
					거래대금_근사값 := s.M거래량 * s.M현재가 / 1000000
					공용.F테스트_참임(테스트, 공용.F오차율(s.M거래대금, 거래대금_근사값) < 10)
				}
				...
				// 등락부호와 등락폭이 뭐 기준인가?
				//공용.F문자열_출력("시각 : %v, 종목코드 : %v", s.M시각, s.M종목_코드)
				//공용.F문자열_출력("전일종가 : %v, 현재가 : %v. 시가 : %v, 고가 : %v, 저가 : %v", 
				//	s.M전일종가, s.M현재가, s.M시가, s.M고가, s.M저가)
				//공용.F문자열_출력("등락부호 : %v, 등락폭 : %v", s.M등락부호, s.M등락폭)
				//공용.F문자열_출력("시가대비 등락부호 : %v, 시가대비 등락폭 : %v", s.M시가대비_등락부호, s.M시가대비_등락폭)
				//공용.F문자열_출력("전일대비 등락부호 : %v, 전일대비 등락폭 : %v", s.M전일대비_등락부호, s.M전일대비_등락폭)
				공용.F메모("c1101 전일 대비 등락폭 문제 있음.")
								
				f테스트_등락부호(테스트, s.M전일대비_등락부호, s.M현재가, s.M전일종가, s.M상한가, s.M하한가)
				//공용.F테스트_같음(테스트, s.M전일종가 + f등락부호2정수(s.M전일대비_등락부호) * s.M전일대비_등락부호, s.M현재가)
				
				if s.M매도_잔량_최우선 > 0 {
					if s.M매도_호가_최우선 < s.M현재가 {
						// 최우선 매도 호가 가 현재가보다 낮은 경우가 있음.
						// 일종의 버그인데, 그래도 그 에러가 너무 크지 않은 지 확인함.
						공용.F메모("최우선 매도 호가가 현재가보다 낮은 경우 발생함.") 
				...
				공용.F테스트_참임(테스트, utf8.ValidString(s.M코스피_코스닥_구분))
				공용.F테스트_참임(테스트, s.M코스피_코스닥_구분 == "코스피" || s.M코스피_코스닥_구분 == "코스닥")
				공용.F테스트_참임(테스트, len(s.M업종명) > 0)
				공용.F테스트_참임(테스트, utf8.ValidString(s.M자본금_규모))
				공용.F테스트_참임(테스트, s.M자본금_규모 == "소형주" || 
					s.M자본금_규모 == "중형주" || s.M자본금_규모 == "대형주", s.M자본금_규모)
				공용.F테스트_참임(테스트, utf8.ValidString(s.M결산월) && 
					strings.Contains(s.M결산월, "결산"))
				공용.F테스트_참임(테스트, utf8.ValidString(s.M시장조치1))
				공용.F테스트_참임(테스트, utf8.ValidString(s.M시장조치2))
				공용.F테스트_참임(테스트, utf8.ValidString(s.M시장조치3))
				공용.F테스트_참임(테스트, utf8.ValidString(s.M시장조치4))
				공용.F테스트_참임(테스트, utf8.ValidString(s.M시장조치5))
				공용.F테스트_참임(테스트, utf8.ValidString(s.M시장조치6))
				...
				공용.F테스트_참임(테스트, s.M대용가 < s.M전일종가 && s.M대용가 > s.M전일종가 / 2)
				...
				공용.F메모("수량 및 금액의 단위에 주의해야 할 듯 함.")
				시가총액_근사값 := s.M현재가 * s.M상장_주식_수량 / 100000000
				공용.F테스트_참임(테스트, 공용.F오차율(s.M시가_총액, 시가총액_근사값) < 10, s.M시가_총액, 시가총액_근사값, 공용.F오차율(s.M시가_총액, 시가총액_근사값)) 
				...
				공용.F테스트_참임(테스트, s.M외국인_지분율 >= 0 && s.M외국인_지분율 <= 100, s.M외국인_지분율)
				
				공용.F메모("결제일의 정의가 뭐지?")
				
				월초 := time.Date(지금.Year(), 지금.Month(), 1, 0, 0, 0, 0, 지금.Location())
				월말 := time.Date(지금.Year(), time.Month(int(지금.Month()) + 1), 1, 0, 0, 0, 0, 지금.Location()).Add(-1 * time.Second)
				공용.F테스트_참임(테스트, s.M결제일.After(월초) && s.M결제일.Before(월말))
				공용.F테스트_참임(테스트, s.M신용잔고_퍼센트 >= 0 && s.M신용잔고_퍼센트 <= 100)
				...
				공용.F테스트_참임(테스트, s.PER > -100 && s.PER < 100, s.PER)
				//공용.F변수값_확인(s.M종목별_신용_한도)
				공용.F테스트_참임(테스트, s.M가중_평균_가격 >= s.M저가 && s.M가중_평균_가격 <= s.M고가)
				공용.F테스트_참임(테스트, 공용.F오차율(s.M상장_주식_수량, s.M상장_주식_수량_1000주_단위 * 1000) < 10)
				공용.F테스트_참임(테스트, s.M추가_상장_주식_수량 >= 0)
				...
				공용.F테스트_참임(테스트, s.M매매_수량_단위 == 0 || s.M매매_수량_단위 == 1 || 
					s.M매매_수량_단위 == 5 || s.M매매_수량_단위 == 10, s.M매매_수량_단위)
