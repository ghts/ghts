...
func TestS콜백_대기(테스트 *testing.T) {
	...
	s1 := New콜백_대기(P접속, "TR코드1", 공용.New질의_가변형(P30초, 공용.P메시지_GET, "인수1"))
	s2 := New콜백_대기(P접속_해제, "TR코드2", 공용.New질의_가변형(P30초, 공용.P메시지_SET, "인수2", "인수3"))

	공용.F테스트_참임(테스트, s1.G식별번호() < s2.G식별번호())

	공용.F테스트_참임(테스트, s1.G유효기간().After(일초전))
	공용.F테스트_참임(테스트, s1.G유효기간().Before(일분후))
	공용.F테스트_같음(테스트, s1.G질의_종류(), P접속)
	...
	공용.F테스트_같음(테스트, s2.G질의_종류(), P접속_해제)
