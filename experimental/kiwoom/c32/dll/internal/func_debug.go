...
func F계좌_수량_테스트() {
	질의 := lib.New채널_질의(lib.New질의값_정수(kt.TR로그인_정보, "", int(kt.P전체_계좌_수량)))
...
func F전체_계좌_번호_테스트() {
	질의 := lib.New채널_질의(lib.New질의값_정수(kt.TR로그인_정보, "", int(kt.P전체_계좌_번호)))
...
func F사용자_ID_테스트() {
	질의 := lib.New채널_질의(lib.New질의값_정수(kt.TR로그인_정보, "", int(kt.P사용자_ID)))
...
func F사용자_이름_테스트() {
	질의 := lib.New채널_질의(lib.New질의값_정수(kt.TR로그인_정보, "", int(kt.P사용자_이름)))
...
func F키보드_보안_상태_테스트() {
	질의 := lib.New채널_질의(lib.New질의값_정수(kt.TR로그인_정보, "", int(kt.P키보드_보안_상태)))
...
func F방화벽_상태_테스트() {
	질의 := lib.New채널_질의(lib.New질의값_정수(kt.TR로그인_정보, "", int(kt.P방화벽_상태)))
...
func F접속_상태_테스트() {
	질의 := lib.New채널_질의(lib.New질의값_기본형(kt.TR접속_상태, ""))
...
func F종목코드_리스트_테스트() {
	...
	//질의 := lib.New채널_질의(lib.New질의값_정수(kt.TR종목코드_리스트, "", P종목코드_전체))

	질의 := lib.New채널_질의(lib.New질의값_정수(kt.TR종목코드_리스트, "", P종목코드_코스피))

	Ch질의 <- 질의

	select {
	case 회신값 := <-질의.Ch회신값:
		if 종목코드_리스트, ok := 회신값.([]string); !ok {
			F체크(lib.F2문자열("F종목코드_리스트() 예상하지 못한 자료형 : '%T'", 회신값))
		} else {
			F체크(lib.F2문자열("F종목코드_리스트() OK. '%v'", 종목코드_리스트))
		}
	case 에러 := <-질의.Ch에러:
