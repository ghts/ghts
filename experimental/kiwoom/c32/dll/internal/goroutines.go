...
func f질의값_처리(질의 *lib.S채널_질의) (에러 error) {
...
func F로그인_정보_처리(질의 *lib.S채널_질의) {
	보관_항목 := New윈도우_메시지_항목()
	S메시지_보관소.S보관(보관_항목)

	질의값, ok := 질의.M값.(*lib.S질의값_정수)
	if !ok {
...
func F종목코드_리스트_처리(질의 *lib.S채널_질의) {
	보관_항목 := New윈도우_메시지_항목()
	S메시지_보관소.S보관(보관_항목)
