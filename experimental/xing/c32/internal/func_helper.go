...
func F질의_처리(w http.ResponseWriter, 질의값 lib.I질의값) (에러 error) {
	...
	if 응답 := f질의_처리_도우미(w, 질의값); 응답.Error() != nil {
		...
		return F회신(w, xt.NewJSON응답(lib.New에러("%v : 예상하지 못한 자료형. %T", 질의값.TR코드(), 응답.V)))
	} else {
	...
	case <-time.After(lib.P1분):
		return F회신(w, xt.NewJSON응답(lib.New에러("F질의_처리() 타임아웃. '%v' '%v'", 질의값.TR코드(), 질의값)))
	case <-lib.Ch공통_종료():
		return nil
	}
}

func f질의_처리_도우미(w http.ResponseWriter, 질의값 lib.I질의값) (응답 *xt.JSON응답) {
	defer lib.S예외처리{}.S실행()
	...
	select {
	case 에러 := <-질의.Ch에러:
		return xt.NewJSON응답(에러)
	case 회신값 := <-질의.Ch회신값:
		return xt.NewJSON응답(회신값)
	case <-time.After(lib.P30초):
		return xt.NewJSON응답(lib.New에러("f질의_처리_도우미() 타임아웃 %v %v", 질의값.TR구분(), 질의값.TR코드()))
	}
}
...
func F회신(w http.ResponseWriter, 값 *xt.JSON응답) error {
	if 바이트_모음, 에러 := json.Marshal(값); 에러 != nil {
		return 에러
	} else if _, 에러 = w.Write(바이트_모음); 에러 != nil {
		return 에러
	} else {
		return nil
	}
