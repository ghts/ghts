...
func F실시간_정보_구독_해지_처리(질의 *lib.S채널_질의) {
	...
	TR코드 := 질의값.(lib.I질의값).TR코드()
	종목코드_모음 := 질의값.(lib.I종목코드_모음).G종목코드_모음()
	...
	case 1:
		...
		단위_길이 = len(종목코드_모음[0])
	default:
		전체_종목코드 = 질의값.(lib.I종목코드_모음).G전체_종목코드()
		단위_길이 = len(종목코드_모음[0])
	}

	if 에러 := 함수(TR코드, 전체_종목코드, 단위_길이); 에러 == nil {
		질의.Ch회신값 <- lib.P신호_OK
	} else {
		질의.Ch에러 <- 에러
	}
...
func F접속_및_로그인_처리(질의 *lib.S채널_질의) {
	접속_처리_잠금.Lock()
