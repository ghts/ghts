...
def 테스트용_주소정보_요청_모듈(P주소_주소정보, P주소_테스트_결과_회신, 테스트_반복횟수):
    #print("주소정보 요청 : 시작")
    
    context = zmq.Context()
    
    가격정보_REQ = context.socket(zmq.REQ)
    가격정보_REQ.connect(P주소_주소정보)
    ...
    예상값_모음 = []
    예상값_모음.append(["가격정보_입수".encode(UTF8), "tcp://127.0.0.1:3004"])
    예상값_모음.append(["가격정보_배포".encode(UTF8), "tcp://127.0.0.1:3005"])
    예상값_모음.append(["테스트_결과_회신".encode(UTF8), "tcp://127.0.0.1:3999"])
    
    테스트_결과 = True
    
    for 반복횟수 in range(테스트_반복횟수):
        리스트 = 예상값_모음[random.randint(0, len(예상값_모음) - 1)]
        질의값 = 리스트[0]
        예상값 = 리스트[1]
        메시지 = [P메시지_구분_일반.encode(UTF8), 질의값]
